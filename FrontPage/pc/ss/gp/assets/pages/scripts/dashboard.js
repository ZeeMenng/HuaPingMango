var Dashboard = function() {
    ...
    return {
        ...
        initCharts: function() {
            ...
            var submitDataString = JSON.stringify(Param.submitData);
            ajaxParameter = {
                "dataType": "json",
                "type": "GET",
                "async": false,
                "url": "/extend/swagger/gp/index/getSiteViews",
 ...
 //筛选的点击事件
                $(".pull-right li a span").click(function () {
                    var txt = $(this).text();
                    $(".dropdown-toggle .selected").text(txt);
                    var typeSel = $(this).attr("type");
                    initLinechart(typeSel)
                });
//封装文章发布数量的方法
            function initLinechart(type){
                ...
                var Param = {
                    submitData : {
                        entityRelated:{
                            type : type,//传查询类型参数 type ，1-人工发布；2-自动采集。
                        }
                    }
                };
                var submitDataString = JSON.stringify(Param.submitData);
                ajaxParameter = {
                    ...
                    "type": "GET",
                    "async": false,
                    "url": "/extend/swagger/gp/index/getPublishArticles",
                    data:  "jsonData="+submitDataString,
                     headers: {
                        "Authorization": 'Bearer ' + JSON.parse(localStorage.getItem("token")).accessToken
                   },
                ...
                $("#site_activities").bind("plothover", function(event, pos, item) {
                    ...
                    if (item) {
                        if (previousPoint2 != item.dataIndex) {
                            ...
                            var x = item.datapoint[0].toFixed(2),
                                y = item.datapoint[1].toFixed(2);
                            showChartTooltip(item.pageX, item.pageY, item.datapoint[0], item.datapoint[1] + '篇');
                        }
                    }
                });
...
function initNumber() {
    //console.log(JSON.parse(localStorage.getItem("token")).accessToken)
        ...
        ajaxParameter1 = {
            ...
            "type": "GET",
            "async": false,
            "url": "/extend/swagger/gp/index/getIndexData",
            data:  "jsonData="+submitDataString,
             headers: {
                "Authorization": 'Bearer ' + JSON.parse(localStorage.getItem("token")).accessToken
           },
            success: function (res) {
                //console.log(res)
                $("#webNum").attr('data-value',res.data[0].sitesNum);
                $("#articleNum").attr('data-value',res.data[0].articleNum);
                $("#organizationNum").attr('data-value',res.data[0].orgNum);
                $("#userNum").attr('data-value',res.data[0].userNum);
            }
        };
        universalAjax(ajaxParameter1);
}
initNumber();
