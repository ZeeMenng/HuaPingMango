...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_PICHANNEL_EDIT
		},
		detailPage : {
			url : RP_PICHANNEL_DETAIL
		},
		addPage : {
			url : RP_PICHANNEL_ADD
		},
		deleteInterface : {
			url : RU_PICHANNEL_DELETE
		},
		deleteListInterface : {
			url : RU_PICHANNEL_DELETELIST
		},
        exportExcelInterface:{
            url:RU_PICHANNEL_EXPORTEXCEL
        }

	};
	var ajaxParam = {
		url : RU_PICHANNEL_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [
            {
                ...
                "linkFunction" : function(event) {
                    var href = RP_PICHANNEL_DETAIL + "?" + RECORD_ID + "=" + event.id;
                    return href;
                }
            },
			{
			"columnName" : "channelPath",
			"columnText" : "访问路径",
			"style" : "text-align:left",
			},
       ]
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_PICHANNEL_DELETE;
