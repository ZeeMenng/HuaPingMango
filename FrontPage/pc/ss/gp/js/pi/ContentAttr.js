 ...
 * @description  CMS内容扩展属性表 相关页面的js方法。
 */
...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_PICONTENTATTR_EDIT
		},
		detailPage : {
			url : RP_PICONTENTATTR_DETAIL
		},
		addPage : {
			url : RP_PICONTENTATTR_ADD
		},
		deleteInterface : {
			url : RU_PICONTENTATTR_DELETE
		},
		deleteListInterface : {
			url : RU_PICONTENTATTR_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_PICONTENTATTR_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_PICONTENTATTR_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_PICONTENTATTR_DELETE;
