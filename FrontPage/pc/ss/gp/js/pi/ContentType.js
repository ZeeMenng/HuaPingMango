 ...
 * @description  CMS内容类型表 相关页面的js方法。
 */
...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_PICONTENTTYPE_EDIT
		},
		detailPage : {
			url : RP_PICONTENTTYPE_DETAIL
		},
		addPage : {
			url : RP_PICONTENTTYPE_ADD
		},
		deleteInterface : {
			url : RU_PICONTENTTYPE_DELETE
		},
		deleteListInterface : {
			url : RU_PICONTENTTYPE_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_PICONTENTTYPE_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_PICONTENTTYPE_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_PICONTENTTYPE_DELETE;
