 ...
 * @description  CMS内容TAG表 相关页面的js方法。
 */
...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_PICONTENTTAG_EDIT
		},
		detailPage : {
			url : RP_PICONTENTTAG_DETAIL
		},
		addPage : {
			url : RP_PICONTENTTAG_ADD
		},
		deleteInterface : {
			url : RU_PICONTENTTAG_DELETE
		},
		deleteListInterface : {
			url : RU_PICONTENTTAG_DELETELIST
		},
		exportExcelInterface:{
			url:RU_PICONTENTTAG_EXPORTEXCEL
		}

	};
	var ajaxParam = {
		url : RU_PICONTENTTAG_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_PICONTENTTAG_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
			"columnName" : "refCounter",
			"columnText" : "被引用的次数",
			"style" : "text-align:left",
			}, 
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_PICONTENTTAG_DELETE;
