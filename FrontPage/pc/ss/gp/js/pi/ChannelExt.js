 ...
 * @description  CMS栏目内容表 相关页面的js方法。
 */
...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_PICHANNELEXT_EDIT
		},
		detailPage : {
			url : RP_PICHANNELEXT_DETAIL
		},
		addPage : {
			url : RP_PICHANNELEXT_ADD
		},
		deleteInterface : {
			url : RU_PICHANNELEXT_DELETE
		},
		deleteListInterface : {
			url : RU_PICHANNELEXT_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_PICHANNELEXT_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_PICHANNELEXT_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
			"columnName" : "finalStep",
			"columnText" : "终审级别",
			"style" : "text-align:left",
			}, 
			 {
			"columnName" : "afterCheck",
			"columnText" : "审核后(1:不能修改删除;2:修改后退回;3:修改后不变)",
			"style" : "text-align:left",
			}, 
			 {
			"columnName" : "isStaticChannel",
			"columnText" : "是否栏目静态化",
			"style" : "text-align:left",
			}, 
			 {
			"columnName" : "isStaticContent",
			"columnText" : "是否内容静态化",
			"style" : "text-align:left",
			}
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_PICHANNELEXT_DELETE;
