...
$(document).ready(function() {
	...
	var pageParam = {
		...
		detailPage : {
			url : RP_PITOPIC_DETAIL
		},
		addPage : {
			url : RP_PITOPIC_ADD
		},
		deleteInterface : {
			url : RU_PITOPIC_DELETE
		},
		deleteListInterface : {
			url : RU_PITOPIC_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_PITOPIC_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_PITOPIC_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_PITOPIC_DELETE;
