 ...
 * @description  CMS内容扩展表 相关页面的js方法。
 */
...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_PICONTENTEXT_EDIT
		},
		detailPage : {
			url : RP_PICONTENTEXT_DETAIL
		},
		addPage : {
			url : RP_PICONTENTEXT_ADD
		},
		deleteInterface : {
			url : RU_PICONTENTEXT_DELETE
		},
		deleteListInterface : {
			url : RU_PICONTENTEXT_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_PICONTENTEXT_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_PICONTENTEXT_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_PICONTENTEXT_DELETE;
