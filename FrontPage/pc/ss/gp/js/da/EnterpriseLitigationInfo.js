...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_DAENTERPRISELITIGATIONINFO_EDIT
		},
		detailPage : {
			url : RP_DAENTERPRISELITIGATIONINFO_DETAIL
		},
		addPage : {
			url : RP_DAENTERPRISELITIGATIONINFO_ADD
		},
		deleteInterface : {
			url : RU_DAENTERPRISELITIGATIONINFO_DELETE
		},
		deleteListInterface : {
			url : RU_DAENTERPRISELITIGATIONINFO_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_DAENTERPRISELITIGATIONINFO_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_DAENTERPRISELITIGATIONINFO_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
			"columnName" : "executionName",
			"columnText" : "被执行人姓名",
			"style" : "text-align:left",
			}, 
			 {
			...
			"style" : "text-align:left",
			 ...
			 {
			"columnName" : "executionCourt",
			"columnText" : "执行法院",
			"style" : "text-align:left",
			}, 
			 {
			"columnName" : "executionObject",
			"columnText" : "案件标的",
			"style" : "text-align:left",
			}
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_DAENTERPRISELITIGATIONINFO_DELETE;
