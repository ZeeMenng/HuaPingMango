...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_DAENTERPRISEPERSONINFO_EDIT
		},
		detailPage : {
			url : RP_DAENTERPRISEPERSONINFO_DETAIL
		},
		addPage : {
			url : RP_DAENTERPRISEPERSONINFO_ADD
		},
		deleteInterface : {
			url : RU_DAENTERPRISEPERSONINFO_DELETE
		},
		deleteListInterface : {
			url : RU_DAENTERPRISEPERSONINFO_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_DAENTERPRISEPERSONINFO_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_DAENTERPRISEPERSONINFO_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_DAENTERPRISEPERSONINFO_DELETE;
