...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_DAENTERPRISETAXINFO_EDIT
		},
		detailPage : {
			url : RP_DAENTERPRISETAXINFO_DETAIL
		},
		addPage : {
			url : RP_DAENTERPRISETAXINFO_ADD
		},
		deleteInterface : {
			url : RU_DAENTERPRISETAXINFO_DELETE
		},
		deleteListInterface : {
			url : RU_DAENTERPRISETAXINFO_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_DAENTERPRISETAXINFO_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_DAENTERPRISETAXINFO_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_DAENTERPRISETAXINFO_DELETE;
