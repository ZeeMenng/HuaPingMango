 ...
 * @description  企业关联企业表，企业上下游关系记录。 相关页面的js方法。
 */
...
$(document).ready(function() {
	...
	var pageParam = {
		...
		editPage : {
			title : "批量修改表单",
			url : RP_DAENTERPRISEUPDOWNSTREAMINFO_EDIT
		},
		detailPage : {
			url : RP_DAENTERPRISEUPDOWNSTREAMINFO_DETAIL
		},
		addPage : {
			url : RP_DAENTERPRISEUPDOWNSTREAMINFO_ADD
		},
		deleteInterface : {
			url : RU_DAENTERPRISEUPDOWNSTREAMINFO_DELETE
		},
		deleteListInterface : {
			url : RU_DAENTERPRISEUPDOWNSTREAMINFO_DELETELIST
		}

	};
	var ajaxParam = {
		url : RU_DAENTERPRISEUPDOWNSTREAMINFO_GETLISTBYJSONDATA,
		type : "GET",
		submitData : {
			"entityRelated" : {
		...
		columnInfo : [ 
			...
			"linkFunction" : function(event) {
				var href = RP_DAENTERPRISEUPDOWNSTREAMINFO_DETAIL + "?" + RECORD_ID + "=" + event.id;
				return href;
			},
			}, 
			 {
	...
	var operationParam = [ {
		...
		"clickFunction" : function(event) {
			layer.confirm('您确定要删除当前记录？', {
				...
				pageParam.deleteInterface.url = RU_DAENTERPRISEUPDOWNSTREAMINFO_DELETE;
