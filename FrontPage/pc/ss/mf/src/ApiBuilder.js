...
class Api {
  ...
  send(data) {
    ...
    var dataStr;
    if (tt) {
      dataStr = JSON.parse(localStorage.getItem("token"));
    }
    if (!simulation) {
        let config;
        if (me._url === "mango/oauth/token") {
            config = {
        ...
        else if(me._url === "mango/oauth/logout"){
            //console.log(dataStr.accessToken)
            let tokens = "Bearer " + dataStr.accessToken;
            config = {
                baseURL: option.baseUrl || baseOption.baseUrl || '',
                url: me._url,
                method: me._method,
      ...
      axios(config)
        .then(response => {
          if (cancelled) return;
          if(response.data.resultCode == '950000'){
