  ...
  componentDidUpdate() {
    ...
    let option = {
      ...
      tooltip: {
        ...
        backgroundColor: 'rgba(25,31,106,0.8)',
        formatter: (params)=> {
          let res = '<div style="min-width:10px;font-size: 14px;line-height: 22px">' + '<h2 style="color:#fff;font-size: 18px;margin-bottom:0;line-height: 30px">' + params[0].name + '</h2>';
          if (params.length > 0) {
            for (let i = 0; i < params.length; i++) {
              res += '<span>' + params[i].seriesName + ': </span>' + '<span>' + params[i].value.toFixed(2) + '万元' + '</span><br/>';
            }
          } else {
            res += '- null'
          }
