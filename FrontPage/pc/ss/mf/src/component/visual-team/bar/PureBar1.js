...
export default class PureBar extends Component {
  ...
  componentDidUpdate() {
    ...
    let yObj2 = {
      ...
      name: '',
      min: 0,
      max: this.state.data.length>0 ? parseFloat(this.state.data.Data[0].growProportion) + 5 : 0,
      axisTick: {
        show: false
      },
      splitNumber: 5,
    ...
    let option = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          let res = '<div style="min-width:10px;font-size: 14px;line-height: 22px">' + '<h2 style="color:#fff;margin-bottom:0;font-size: 18px;line-height: 30px">' + params[0].name + '</h2>';
          if (params.length > 0) {
            for (let i = 0; i < this.state.data.Data.length; i++) {
              if (this.state.data.Data[i].strainsText == params[0].name) {
                res += '种植面积<span>' + params[0].seriesName + ': </span>' + '<span>' + this.state.data.Data[i].growArea + unitArr[0] + '</span><br/><span>占总面积：' + this.state.data.Data[i].growProportion + '</span>';
              }
            }
          } else {
            res += '- null'
...
// if (params.length > 0) {
//   for (let i = 0; i < params.length; i++) {
//     if (this.state.data.Data[i].strainsText == params.name) {
//       console.log(params[i])
//       res += '种植面积<span>: </span>' + '<span>' + this.state.data.Data[i].growArea + unitArr[i] + '</span><br/><span>占总面积：' + this.state.data.Data[i].growProportion + '</span>';
//     }
//   }
// }