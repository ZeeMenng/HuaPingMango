...
export default class CircleBig extends React.Component {
  ...
  render() {
    ...
    if(Object.keys(newData).length !== 0){//首页日均价的保留一位小数处理
      wholeSaleNum = Number(newData.wholeSale.data).toFixed(1);
      retailNum = Number(newData.retail.data).toFixed(1);
      refreshDate=Number(newData.retail.data).toFixed(1);
    }

    let number = {
      wholeSale: wholeSaleNum || 0,
      retail: retailNum || 0,
      ...
      <div style={{width: '655px', height: '475px', position: 'absolute', left: '114px', top: '65px'}}>
        <div style={{width: '655px', height: '242px'}}>
          <div style={{
            ...
            <p style={{...me.wholeSalePrice, color: '#00f6ff'}}>批发价</p>

            <p style={me.price}>
              <Word num={number.wholeSale} fontSize={'55px'} color1={'#9affff'} color2={'#21d1f5'}/>
            </p>
            <p style={{...me.weightThing,color: '#41beff'}}>元／公斤</p>
          ...
          <div style={{
            ...
            <p style={{...me.wholeSalePrice, color: '#00ff6c'}}>电商价</p>

            <p style={me.price}>
              <Word num={number.retail} fontSize={'55px'} color1={'#afffd9'} color2={'#2af594'}/>
            </p>
            <p style={{...me.weightThing,color: '#00ff6c'}}>元／公斤</p>
