...
class CircleSmall extends Component {
  ...
  render() {
    ...
    return (
      <div ref={'boxes'} style={this.props.style}>
        ...
        <p style={{
          background: `url(${circleTop}) no-repeat center center`,
  ...
  componentDidMount() {
    ...
    me._createTextStatic(-18, 30, 15, me.colors, '元/公斤');
  }
  ...
  componentDidUpdate() {
    ...
    if (Object.keys(newData).length !== 0) {
      let num = isNaN(newData[me.typeData][me.times]) == true ? '--' :  Number(newData[me.typeData][me.times]).toFixed(1);
      //console.log(num)
      let x;
      if (String(num).indexOf('.') === 1) {
        x = -24;
      } else if (String(num).indexOf('.') === 2) {
        x = -29;
      }

      me._createText(x, 7, 30, `url(#${me.ids})`, num);
