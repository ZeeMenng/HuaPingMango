      ...
      me._option = {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            ...
            let predictData = undefined;
            let titleName = '';
            if (params.length > 0) {
              params.forEach((item, index) => {
                if (item.seriesName.indexOf('实际') > -1) {
                  actualdata = item.value;
                } else if (item.seriesName.indexOf('预测') > -1) {
                  predictData = item.value;
                  titleName = item.axisValue;
                }
              });
            } else {
              return;
            }
            let res = '<div style="min-width:10px;font-size: 14px;line-height: 22px">' + '<h2 style="color:#dbfcff;font-size: 18px;line-height: 30px">' + titleName + '</h2>';
            if (actualdata) {
              res += `<div>全国实际${actualdata}${me._objData.yAxisName || '元/公斤'}</div>`;
            }
            if (predictData && predictData !== actualdata &&predictData!='-') {
              res += `<div>全国预测${predictData}${me._objData.yAxisName || '元/公斤'}</div>`;
            }
        ...
        grid: {
          ...
          bottom: me._objData.gridBottom || '10%',
          containLabel: true
        },
        dataZoom: [
          {
