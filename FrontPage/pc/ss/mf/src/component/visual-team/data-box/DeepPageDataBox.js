...
import arrowUp from './arrow-up.gif';
import arrowDown from './arrow-down.gif';
...
class DeepPageDataBox extends Component {
  ...
  render() {
    ...
    let titleName = this.props.title || ['芒果汁', '芒果干', '芒果酱', '芒果醋', '芒果酒'];
    let arrName = ['产量', '产值'];
                      ...
                      <div style={{ width: '150px', height: '80px', border: '1px solid #04c0ff', marginBottom: '5px' }} key={'_zll' + i}>
                        ...
                        <div style={{
                          ...
                          <p style={{ width: '100%', height: '37px', lineHeight: '40px', marginBottom: 0 }}>
                            <Word fontSize={'35px'} color1={'#fff'} color2={'#00eaff'} num={v=='产量' ? this.state.value[name]? (this.state.value[name][v] / 10000000).toFixed(2) : 0  : this.state.value[name] ? (this.state.value[name][v] / 100000000).toFixed(2) : 0} />
                            <Word fontSize={'16px'} color1={'#fff'} color2={'#00eaff'}  num={v=='产量' ? '万吨' : '亿元'} />
                          </p>
