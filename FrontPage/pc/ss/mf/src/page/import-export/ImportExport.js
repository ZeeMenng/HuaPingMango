...
class ImportExport extends React.Component {
  ...
  initWorldMap = (sImport, unit, year)=>{
    let sImportVal = sImport || 1
    let params = {
      jsonData: {
        "entityRelated": {
          "queryPattern": sImportVal,
          "queryType": unit || "1",
          "queryTime": year || this.state.showCon
        },
        "orderList": [
    ...
    api.worldMap.send(params).then(res => {
      ...
      if (res.isSuccess){
        ...
        this.setState({
          ...
          //console.log('1:'+this.state.worldMapData)
        })
      }else{
        this.setState({
          worldMapData: dataArr
  ...
  initSelect = () => {
    ...
    api.getTimeData.send(getTimeDataParams).then((res) => {
      ...
      this.setState({
        timeData: res.data || this.state.showCon,
        timeNow: res.data[3] || this.state.showCon
      })

      //  前十贸易国
      this.initTradeTop10(1, 1, this.state.showCon)
      this.initWorldMap(1, 1, this.state.showCon)
    })
  }
  //  地图前十贸易国进出口
  totalTradeTop10 = (e)=>{
    ...
    return (
      <div className="import-container">
          ...
          <div className="import-select-container">
            <div className="import-change">
              <Tabs defaultActiveKey="1" onChange={this.totalTradeTop10}>
                <TabPane tab="中国进口" key="1"></TabPane>
                <TabPane tab="中国出口" key="2"></TabPane>
              </Tabs>
            </div>
            <div className="import-change">
            ...
            <Select1 _pullDownMes={this._totalTradeTop10} nameArr={this.state.timeData} showCon={this.state.showCon} style={{
              position: 'absolute',
              top: '20px',
              left: '592px',
              width: '110px',
        ...
        <div className="right-chart">
          ...
          <div className="import-price-chart">
            <FreshTop10
              ...
              margins="80"
              tooltipUnit={this.state.tooltipUnit}
              textStyle={{
                color: '#FFF',
        ...
        <div className="right-chart">
          ...
          <div className="import-price-chart">
            <RiseFall data={this.state.riseFallData} sort={true} unit={'单位：'+this.state.unit}/>
          </div>
        </div>
      </div>
    )
