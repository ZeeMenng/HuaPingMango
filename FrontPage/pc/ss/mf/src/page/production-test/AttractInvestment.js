      ...
      <div className={'attract-wrapper'}>
        <div className={'attract-header'}>
          <div className={'attract-title'}>
            <HeaderText content={`为您推荐`+this.state.total+`家相关企业`} top={'124px'} left={'167px'} fontSize={'0.22rem'}/>
          </div>
        ...
        <LocaleProvider locale={zhCN}>
                   <Pagination showTotal={(total, range) => `页${this.state.pageIndex}/共${Math.ceil(this.state.total/9)}页，每页9条`} showQuickJumper defaultPageSize={9} defaultCurrent={1} total={this.state.total} onChange={this.onChange} />
        </LocaleProvider>
