...
import * as internetApi from "../api/page-data";
...
class InternetThing extends React.Component {
  ...
  constructor() {
    ...
    const me = this;
    this.state = {
      earlyWarningStyle : '3',//默认预警类别--总预警
      earlyWarningTime : `${new Date().getFullYear()}-${new Date().getMonth()}`,//默认预警类别--预警时间
      earlyWarningStyleList : [{code:0,id:0,text:'总预警'}],//预警类别列表
      ...
      getTimeQuantumDataByDeviceObj : {
          pageNum : '0',
          pageSize : '0',
      },//页码
      startTime : (new Date()).toLocaleDateString(),//历史数据开始时间
      endTime : (new Date()).toLocaleDateString(),//历史数据结束时间
      ...
      <div className={'InterHomeStyle'}>
          ...
          <div>
              <Modal
                  ...
                  title={this.state.getBaseInfoByDeviceObj.baseName}
                      ...
                      <div className={'dialogHead'}>
                          <ul>
                              <li>
                                  ...
                                  <dl>
                                      <dt>种植面积：</dt>
                                      <dd>{(this.state.getBaseInfoByDeviceObj.areaSum / 6670000)}万亩</dd>
                                  </dl>
                                  <dl>
                                      <dt>品种：</dt>
                                  ...
                                  <dl>
                                      <dt>设备编号：</dt>
                                      <dd>{this.state.deviceId}</dd>
                                  </dl>
                              </li>
                          </ul>
                      </div>
                      <div className={'dialogContent'}>
                          <Tabs defaultActiveKey="1"  className={'dialogContentTop'} onChange={this.callback}>
                                              ...
                                              <ul>
                                                  ...
                                                  <li>
                                                      <dl>
                                                          ...
                                                          <dd>
                                                              <p>土壤湿度</p>
                                                              <span>{this.state.getActualTimeDataByDeviceObjData['787']}</span>
                                                          </dd>
                                                      </dl>
                                                  </li>
                                                  <li>
                                  ...
                                  <div className={'historyData'}>
                                      <div className={'historyDataTime'}>
                                          ...
                                          <LocaleProvider locale={zhCN}>
                                              <RangePicker defaultValue={[moment((new Date()).toLocaleDateString(), dateFormat), moment((new Date()).toLocaleDateString(), dateFormat)]}
                                                           format={dateFormat}  showTime
                                                           dropdownClassName="dialogRangePicker" onChange={this.dateChange} />
                                          </LocaleProvider>
                                      </div>
                                      <div className={'historyDataList'}>
                                          ...
                                          <LocaleProvider locale={zhCN}>
                                              <Pagination showQuickJumper defaultCurrent={1} total={this.state.getTimeQuantumDataByDeviceList.totalCount == undefined ? 0 : this.state.getTimeQuantumDataByDeviceList.totalCount}
                                                          showTotal={(total, range) => `页${this.state.getTimeQuantumDataByDeviceObj.pageSize == 0 ? 0 : this.state.getTimeQuantumDataByDeviceObj.pageNum} / 共${this.state.getTimeQuantumDataByDeviceObj.pageSize}页  每页10条`}
                                                          onChange={this.getTimeQuantumDataByDeviceList} />
                                          </LocaleProvider>
          ...
          {this.state.earlyWarningBtnShow && <div className={'earlyWarningBtn'}>
              <button onClick={this.isFull}>{this.state.isFull?'退出全屏':'地图全屏'}</button>
              {/* <button onClick={this.earlyWarningShow}>{this.state.earlyWarningShow?'点击关闭预警':'点击展示预警'}</button> */}
          </div>
          }
          ...
          <div className={ this.state.isFull == true ?  'InterBottomStyle InterBottomStyleActive' : 'InterBottomStyle'}>
              ...
              <div className={'InterBottomL'}>
                  <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '40px'}}>
                      ...
                      <Select dropdownClassName="InternetSelect2" value={this.state.earlyWarningTime}
  ...
  showModal = (id) => {
      ...
      this.getTimesDataByDeviceId(this.state.deviceId, this.state.selectInternetStyleIndex);//默认土壤温度
    }
  //查询 过去24小时的监测数据
  ...
  getTimeQuantumDataByDeviceList = (pageNum) => {
      ...
      this._tokens.push(internetApi.getTimeQuantumDataByDeviceId.send(params).then((res)=>{
          ...
          let obj = {
              pageNum : pageNum,
              pageSize : Math.floor(res.totalCount / 10),
          }
          this.setState({
              getTimeQuantumDataByDeviceObj : obj
          })
      }));
  }
  //24小时监测数据警戒线
  ...
  earlyWarningShow = (data) => {
      ...
      if(!this.state.earlyWarningShow){
          ...
          this._tokens.push(internetApi.getEarlyWarningStyleList.send().then((res)=>{
              ...
              this.setState({
                  earlyWarningStyleList : this.state.earlyWarningStyleList.concat(res.data)
              })
          }));
          //预警时间
          let params = {
          ...
          this.selectEarlyWarning('3','2018-05');
