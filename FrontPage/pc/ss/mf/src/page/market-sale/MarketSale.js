...
import Select2 from '../../component/web-team/select/Select1';
import Panel from '../../component/visual-team/panel/Panel';
import Title from '../../component/visual-team/title/Title';
import PureBar from '../../component/visual-team/bar/ManggoValue';
...
//芒果品种消费量偏好
import MangoPie from '../../component/visual-team/pie/MangoPieType';
import MangoPie1 from '../../component/visual-team/pie/MangoPie1';
...
class MarketSale extends React.Component {
  ...
  showYear() {
    let startyear = new Date().getFullYear(),endyear =startyear-4, years=[];
    ...
    return (
      <div>
        <div style={{width: '840px', height: '620px', position: 'absolute', top: '126px', left: '180px'}}>
          <div style={{position: 'absolute', left: '0', top: '450px', width: '250px', height: '125px'}}>
            <p style={{paddingLeft: '10px',fontSize:'14px', color: '#46ebff', height: '16px', lineHeight: '16px'}}>流向全国芒果消费量排名TOP5</p>
            <ChinaMapBar ref={'chinaMapBar'} style={{marginTop: '10px', width: '280px', height: '100px'}}/>
          </div>
          ...
          <ul style={{width: '100%', height: '40px', position: 'absolute', top: '60px'}}>
            <Select2 _pullDownMes={this._pullDownMes.bind(this, 1)} nameArr={this.state.timeDate} showCon={this.state.showCon} width={80}
                    style={{position: 'absolute'}}/>
            <Select _pullDownMes={this._pullDownMes.bind(this, 2)} nameArr={['鲜芒果', '加工芒果']} width={80}
          ...
          <div style={{width: '100%', height: '550px', marginTop: '-30px'}}>
        ...
        <Panel height={300} width={440} left={'1440px'} top={'125px'}>
          ...
          <ul style={{position: 'absolute', top: '60px', width: '100%', height: '35px'}}>
            <Select _pullDownMes={this._pullDownMes.bind(this, 5)} nameArr={['鲜芒果', '加工芒果']} width={80}
                    style={{position: 'absolute', left: '20px'}}/>
            <Select _pullDownMes={this._pullDownMes.bind(this, 6)} nameArr={['销售量', '销售金额']} width={80}
                    style={{position: 'absolute', left: '140px'}}/>
          </ul>
          <div style={{position: 'absolute', width: '100%', height: '200px', top: '96px'}}>
            <CusomForeast ref={'consumForecast'}
                          style={{width: '425px', height: '200px', position: 'absolute', left: '15px', top: '0px'}}/>
          </div>
        </Panel>
        ...
        <Panel height={300} width={440} left={'1440px'} top={'435px'}>
          ...
          <div style={{position: 'absolute', width: '100%', height: '230px', top: '63px'}}>
            <PureBar ref={'manggoValue'}
                     style={{width: '425px', height: '220px', position: 'absolute', left: '15px', top: '10px'}}/>
          </div>
        </Panel>

        <div style={{
          borderTop: '1px solid #3353b5',
          width: '370px',
          height: '600px',
          position: 'absolute',
          left: '1033px',
        ...
        <Panel height={285} width={1720} left={'160px'} top={'755px'}>
          ...
          <p ref={'hurley1'} style={{
            ...
            fontSize: '16px',
            width: '1500px'
          }}>结论：每公斤价格为10-15元贵妃芒最受消费者青睐。果径为8-9cm，单价为5-15元/公斤的芒果最受消费者青睐。</p>
          <Select _pullDownMes={this.changeValue.bind(this)} nameArr={['单向偏好', '组合偏好']} width={80}
                  style={{position: 'absolute', left: '1600px'}}/>
          <div ref={'hurley2'}
               style={{display: 'none', width: '1720px', height: '227px', position: 'absolute', top: '53px'}}>
            <CombineHobby ref={'combineHobby'}/>
          </div>
          <div ref={'hurley3'}
               style={{display: 'block', width: '1720px', height: '227px', position: 'absolute', top: '53px'}}>

            <MangoPie ref={'mangoSaleRefs'} style={this.mangoSaleStyle}/>
            <LikePrice ref={'mangoPriceRefs'} style={this.mangoPriceStyle}/>
            <MangoPie1 style={this.mangoWhreeSale} ref={'mangoWhreeSaleRefs'}/>
  ...
  componentDidMount() {
    ...
    me._tokens.push(api.mangoSaleJson.send({
      ...
      res.data.map((v, i) => {
        data.push({
          names: v.sale_channel_type_text,
          vals: (v.amount * 0.001).toFixed(1),
          unit: "吨"
        })
      });
      let obj = {
      ...
      me.refs.mangoWhreeSaleRefs.setData(obj);
    }));
    // 芒果规格偏好
    me._tokens.push(api.mangoSaleJson.send({
      jsonData: JSON.stringify({
      ...
      res.data.map((v, i) => {
        data.push({
          names: v.weight_specification_text,
          vals: (v.amount / 1000).toFixed(1),
          unit: "吨"
        })
      });
      let obj = {
    ...
    me._tokens.push(api.map1.send({
      jsonData: JSON.stringify({
        "entityRelated": {
          ...
          "queryTime": this.state.showCon //查询日期 2018-05
        },
        "page": {
          "pageIndex": 1,
          "pageSize": 100
      ...
      for (let i = 0; i < response.data.length; i++) {
        serchMax.push(response.data[i].unit);
      }
      ...
      max = 1.7 * serchMax[serchMax.length - 1];

      for (let i = 0; i < response.data.length; i++) {
        arr.push({
          "fromRegionCode": response.data[i].sale_region_code,
          "touristNum": response.data[i].sale_region_text,
          "value": (Number(response.data[i].unit) / max)
        })
      }
    ...
    me._tokens.push(api.manggoValue1.send({
      ...
      me._tokens.push(api.manggoValue2.send({
        ...
        for (let i = 0; i < res1.data.length; i++) {
          ...
          yData1.push(Number(res1.data[i].output_value_unit) / 10000 || 0);
        }
        for (let i = 0; i < res2.data.length; i++) {
          yData2.push(Number(res2.data[i].output_value_unit) / 10000 || 0);
        }
    ...
    me._tokens.push(api.saleResult.send({
      ...
      let arr = [];
      if(res.isSuccess){
          arr.push(res.data[0], res.data[2], res.data[1], res.data[3]);
      }else{
        arr=[
          {'mount':0,'onYear':0,'flag1':0,'onMonth':0,'name':'鲜果销量'},
          {'mount':0,'onYear':0,'flag2':0,'onMonth':0,'name':'鲜果产值'},
          {'mount':0,'onYear':0,'flag3':0,'onMonth':0,'name':'加工品销量'},
          {'mount':0,'onYear':0,'flag4':0,'onMonth':0,'name':'加工品产值'}
        ];
      }

      me.refs.saleResult.setData(arr);
    }));
    ...
    me._tokens.push(api.combineHobby.send({
      ...
      me._tokens.push(api.combineHobby.send({
        ...
        for (let i = 0; i < res1.data.data.length; i++) {
          arr1.push({
            "name": res1.data.data[i].name,
            "lev1": res1.data.data[i].lev1 === '-' ? '-' : parseInt(Number(res1.data.data[i].lev1/1000)),
            "lev2": res1.data.data[i].lev2 === '-' ? '-' : parseInt(Number(res1.data.data[i].lev2/1000)),
            "lev3": res1.data.data[i].lev3 === '-' ? '-' : parseInt(Number(res1.data.data[i].lev3/1000)),
            "lev4": res1.data.data[i].lev4 === '-' ? '-' : parseInt(Number(res1.data.data[i].lev4/1000)),
            "lev5": res1.data.data[i].lev5 === '-' ? '-' : parseInt(Number(res1.data.data[i].lev5/1000))
          })
        }
        for (let i = 0; i < res2.data.data.length; i++) {
          arr2.push({
            "name": res2.data.data[i].name,
            "lev1": res2.data.data[i].lev1 === '-' ? '-' : parseInt(Number(res2.data.data[i].lev1/1000)),
            "lev2": res2.data.data[i].lev2 === '-' ? '-' : parseInt(Number(res2.data.data[i].lev2/1000)),
            "lev3": res2.data.data[i].lev3 === '-' ? '-' : parseInt(Number(res2.data.data[i].lev3/1000)),
            "lev4": res2.data.data[i].lev4 === '-' ? '-' : parseInt(Number(res2.data.data[i].lev4/1000)),
            "lev5": res2.data.data[i].lev5 === '-' ? '-' : parseInt(Number(res2.data.data[i].lev5/1000))
          })
        }
    ...
    me._tokens.push(api.consumForecast1.send({
      ...
      for (let i = 0; i < res.data.mfFreshList.length; i++) {
        ...
        yArr.push(res.data.mfFreshList[i].value / 10000000);
      }
    ...
    me._tokens.push(api.chinabar1.send({
      jsonData: JSON.stringify({
        "entityRelated": {
          "queryType": "1",      //查询类型 1：销售量，2：销售额。
          "queryTime": this.state.num1 //查询日期 2018-05
        },
        "page": {
          "pageIndex": 1, //需要传此字段
          "pageSize": 5   //需要传此字段
  ...
  componentDidUpdate() {
    ...
    me._tokens.push(api[flag1].send({
      ...
      for (let i = 0; i < res.data[manggo].length; i++) {
        xArr.push(res.data[manggo][i].time);
        yArr.push(res.data[manggo][i].value / scale);
      }
    ...
    me._tokens.push(api[f1].send({
      ...
      for (let i = 0; i < response.data.length; i++) {
        arr.push({
          "fromRegionCode": response.data[i].sale_region_code,
          "touristNum": response.data[i].sale_region_text,
          "value": (Number(response.data[i].sale_amount_unit) / max)
        })
      }
      let centerLonLat = [100.25, 26.86];
