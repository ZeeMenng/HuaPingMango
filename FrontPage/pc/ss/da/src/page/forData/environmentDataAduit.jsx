...
export default class EnvironmentDataAduit extends Component {
    constructor(props){
        super(props)
        this.state = {
    ...
    initList=(id)=>{
        let v = id.split('-')[0]
        api.altitudeOneData.send({id:v}).then(res=>{
            console.log(res)
            if(res.isSuccess){
                let v = res.data;
                this.setState({
            ...
            <div className="forData">
                <ul className="form-search form-aduit clearfix">
                    ...
                    <li>
                        ...
                        <span>{altitude?altitude+'米':''}</span>
                    </li>
                    <li>
                        <label htmlFor="">平均气温：</label>
                        <span>{averageTemperature?averageTemperature+'℃':''}</span>
                    </li>
