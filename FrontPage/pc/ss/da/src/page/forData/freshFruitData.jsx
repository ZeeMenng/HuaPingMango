...
export default class FreshFruitData extends Component {
    constructor(props) {
        super(props)
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daIndustryFresh/importExcel',
                headers: {
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/indexReport/freshFruitData/freshFruitDataAduit/${text}-${record.key}`}>
                                    <Icon 
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/indexReport/freshFruitData/freshFruitDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
    ...
    delListFn = (delArr) =>{
        ...
        api.freshFruitDelList.send(params).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    confirmDel=(e)=>{
        // console.log(e);
        api.freshFruitDel.send({id:e}).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    initAltitudeList=(current)=>{
        ...
        api.freshFruitList.send(params).then(res=>{
            console.log(res)
            let tableData=[];
            if(res.isSuccess){
                res.data.forEach((v,i)=>{
        ...
        return (
            <div className="forData">
                ...
                <div className="btn-type">
                    <NavLink to="/indexReport/freshFruitData/freshFruitDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
