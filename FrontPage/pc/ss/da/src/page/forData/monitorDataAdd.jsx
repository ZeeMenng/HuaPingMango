...
export default class MonitorDataAdd extends Component {
    constructor(props) {
        super(props)
        console.log()
        this.state = {
    ...
    initInput = (id) => {
        // console.log(id)
        api.monitorDataSearch.send({ id: id }).then(res => {
            console.log(res)
            if (res.isSuccess) {
                let v = res.data;
                this.setState({
    ...
    //  保存与修改方法
    formSaveFn = () => {
        const { isAdd, checkDate, checkOrg, checkPersonnel, checkProject, checkResult, cropTypeCode, cropTypeText, areaLatitudeTypeCode,
            areaLatitudeTypeText, startTime, dataSources, dataTimeTypeCode, dataTimeTypeText, regionId, regionName, sourceChannelTypeCode,
            sourceChannelTypeText, name, sampleNo, sampleName, strainsCode, strainsText,checkProjectCode
        } = this.state
        let params = {
            "checkDate": checkDate,//检测时间
            "checkOrg": checkOrg,//检测机构
            "checkPersonnel": checkPersonnel,//检测人员
            "checkProject": checkProject,//检测项目
            "checkResult": checkResult,//检测结果
        ...
        if (Number(isAdd) === 1) {
            //  新增
            // console.log(params)
            api.monitorDataAdd.send(params).then(res => {
                // console.log(res)
                if(res.isSuccess){
            ...
            api.monitorDataFix.send(params).then(res => {
                // console.log(res)
                if (res.isSuccess) {
    ...
    //  采样日期
    checkSelect = (date, dateString) => {
        // console.log(date, dateString);
        this.setState({
            checkDate: dateString + ' 00:00:00'
        })
    }
    componentDidMount() {
            ...
            <div className="forData">
                <ul className="form-search form-aduit clearfix">
                    ...
                    <li>
                        ...
                        <DatePicker locale={locale} value={checkDate ? moment(new Date(checkDate)) : null} onChange={this.checkSelect} style={{ width: '65%' }} />
                    </li>
                    <li>
                    ...
                    <li>
                        ...
                        <Input onChange={this.inputHandle} data-type="checkOrg" style={{ width: '65%' }} value={checkOrg} defaultValue={checkOrg} />
                    </li>
                    <li>
                        <label htmlFor="">检测人员：</label>
                        <Input onChange={this.inputHandle} data-type="checkPersonnel" style={{ width: '65%' }} value={checkPersonnel} defaultValue={checkPersonnel} />
                    </li>
                    <li style={{display: 'flex', alignItems: 'center'}}>
                        <label htmlFor="">检测结果：</label>
                        <TextArea onChange={this.inputHandle} data-type="checkResult" style={{ width: '65%' }} value={checkResult} defaultValue={checkResult} />
                    </li>
                    <li>
