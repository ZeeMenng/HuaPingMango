...
export default class PlantingData extends Component {
    constructor(props) {
        super(props)
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daGrowYield/importExcel',
                headers: {
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/forData/plantingData/plantingDataAduit/${text}-${record.key}`}>
                                    <Icon
                                        type="credit-card"
                            ...
                            <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/forData/plantingData/plantingDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
    ...
    delListFn = (delArr) => {
        ...
        api.plantingDelList.send(params).then(res => {
            // console.log(res)
            if (res.isSuccess) {
    ...
    confirmDel = (e) => {
        // console.log(e);
        api.plantingDel.send({ id: e }).then(res => {
            // console.log(res)
            if (res.isSuccess) {
    ...
    initAltitudeList = (current) => {
        ...
        api.plantingList.send(params).then(res => {
            console.log(res)
            let tableData = [];
            if (res.isSuccess) {
        ...
        return (
            <div className="forData">
                ...
                <div className="btn-type">
                    <NavLink to="/forData/plantingData/plantingDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
