...
export default class ProcessValueData extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daIndustryProcess/importExcel',
                headers: {
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/indexReport/processValueData/processValueDataAduit/${text}-${record.key}`}>
                                    <Icon 
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/indexReport/processValueData/processValueDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
     ...
     confirmDel=(e)=>{
        // console.log(e);
        api.processValueDataDel.send({id:e}).then(res=>{
            // console.log(res)
            if(res.isSuccess){
     ...
     delListFn = (delArr) =>{
        ...
        api.processValueDataDelList.send(params).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    initAltitudeList=(current)=>{
        ...
        console.log(params)
        api.processValueDataList.send(params).then(res=>{
            console.log(res)
            let tableData=[];
            if(res.isSuccess){
                res.data.forEach((v,i)=>{
        ...
        return (
            <div className="forData">
                ...
                <div className="btn-type">
                    <NavLink to="/indexReport/processValueData/processValueDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
