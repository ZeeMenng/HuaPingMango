...
import { Select, DatePicker, Upload, Cascader, Input, Table, Button, Icon, Tooltip, Popconfirm, message } from 'antd';
...
export default class MonitorData extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daCheckTesting/importExcel',
                headers: {
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/viewData/monitorData/monitorDataAduit/${text}-${record.key}`}>
                                    <Icon
                                        type="credit-card"
                            ...
                            <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/viewData/monitorData/monitorDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
    ...
    confirmDel = (e) => {
        // console.log(e);
        api.monitorDataDel.send({ id: e }).then(res => {
            // console.log(res)
            if (res.isSuccess) {
    ...
    initAltitudeList = (current) => {
        ...
        api.monitorDataList.send(params).then(res => {
            console.log(res)
            let tableData = [];
            if (res.isSuccess) {
        ...
        return (
            <div className="forData">
                ...
                <div className="btn-type">
                    <NavLink to="/viewData/monitorData/monitorDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
