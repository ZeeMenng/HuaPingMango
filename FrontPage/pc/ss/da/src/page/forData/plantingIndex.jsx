...
export default class PlantingIndex extends Component {
    constructor(props) {
        super(props)
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daEcologyNormInfo/importExcel',
                headers: {
                ...
                onChange: (info) => {
                    ...
                    if (info.file.status === 'done') {
                        ...
                        if (isSuccess) {
                            ...
                            //  刷新列表
                            this.initList(this.state.current)
                        } else {
                            message.error("上传失败！");
                        }
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/forData/plantingIndex/plantingIndexAduit/${text}-${record.key}`}>
                                    <Icon 
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/forData/plantingIndex/plantingIndexAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
    ...
    delListFn = (delArr) =>{
        ...
        api.plantingIndxDelList.send(params).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    confirmDel=(e)=>{
        // console.log(e);
        api.plantingIndxDel.send({id:e}).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    initList=(current)=>{
        ...
        api.plantingIndxList.send(params).then(res=>{
            console.log(res)
            let tableData=[];
            if(res.isSuccess){
        ...
        return (
            <div className="forData">
                ...
                <div className="btn-type">
                    <NavLink to="/forData/plantingIndex/plantingIndexAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
