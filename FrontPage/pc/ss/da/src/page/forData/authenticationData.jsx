...
import { Select,Upload, Cascader,DatePicker, Input, Table, Button, Icon, Tooltip, Popconfirm, message } from 'antd';
...
export default class AuthenticationData extends Component {
    constructor(props) {
        super(props)
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daIdentificationInfo/importExcel',
                headers: {
                ...
                onChange: (info) => {
                    ...
                    if (info.file.status === 'done') {
                        ...
                        if (isSuccess) {
                            ...
                            //  刷新列表
                            this.initTableList(this.state.current)
                        } else {
                            message.error("上传失败！");
                        }
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/forData/authenticationData/authenticationDataAduit/${text}-${record.key}`}>
                                    <Icon 
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/forData/authenticationData/authenticationDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
    ...
    delListFn = (delArr) =>{
        ...
        api.authenticationDelList.send(params).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    confirmDel=(e)=>{
        console.log(e);
        api.authenticationDel.send({id:e}).then(res=>{
            console.log(res)
            if(res.isSuccess){
    ...
    initTableList=(current)=>{
        ...
        api.authenticationList.send(params).then(res=>{
            console.log(res)
            let tableData=[];
            if(res.isSuccess){
        ...
        return (
            <div className="forData">
                ...
                {/* <li>
                    ...
                    <li>
                        <Button onClick={this.initTableList.bind(undefined,1)} type="primary" className="fr" style={{ marginRight: "12px" }} icon="search">检索</Button>
                    </li>
                </ul>
                <div className="btn-type">
                    <NavLink to="/forData/authenticationData/authenticationDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
