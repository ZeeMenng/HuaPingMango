...
import { Select, DatePicker, Cascader, Input, Table, Button, Icon, Tooltip, Popconfirm, message, Upload } from 'antd';
...
export default class ProcessData extends Component {
    constructor(props) {
        super(props)
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daProcess/importExcel',
                headers: {
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/forData/processData/processDataAduit/${text}-${record.key}`}>
                                    <Icon
                                        type="credit-card"
                            ...
                            <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/forData/processData/processDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
    ...
    delListFn = (delArr) => {
        ...
        api.processDataDelList.send(params).then(res => {
            // console.log(res)
            if (res.isSuccess) {
    ...
    confirmDel = (e) => {
        // console.log(e);
        api.processDataDel.send({ id: e }).then(res => {
            // console.log(res)
            if (res.isSuccess) {
    ...
    initAltitudeList = (current) => {
        ...
        api.processDataList.send(params).then(res => {
            console.log(res)
            let tableData = [];
            if (res.isSuccess) {
        ...
        return (
            <div className="forData">
                <ul className="form-search clearfix">
                    ...
                    <li>
                        <Button type="primary" onClick={this.initAltitudeList.bind(undefined, current)} className="fr" style={{ marginRight: "12px" }} icon="search">检索</Button>
                    </li>
                </ul>
                <div className="btn-type">
                    <NavLink to="/forData/processData/processDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
