...
import { Select,Cascader, DatePicker, Upload, Input, Table, Button, Icon, Tooltip, Popconfirm, message } from 'antd';
...
export default class EnvironmentData extends Component {
    constructor(props) {
        super(props)
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01+'/extend/swagger/da/daAltitudeInfo/importExcel',
                headers: {
            ...
            columns: [
                ...
                { title: '采集日期', dataIndex: '1'},
                { title: '区域维度',dataIndex: '2'},
                { title: '地理区域',dataIndex: '3'},
                { title: '数据日期',dataIndex: '4'},
                { title: '名称', dataIndex: '5'},
                { title: '海拔（米）',dataIndex: '6'},
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/forData/environmentData/environmentDataAduit/${text}-${record.key}`}>
                                    <Icon 
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/forData/environmentData/environmentDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
    ...
    delListFn = (delArr) =>{
        ...
        api.altitudeDelList.send(params).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    confirmDel=(e)=>{
        // console.log(e);
        api.altitudeDel.send({id:e}).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    initAltitudeList=(current)=>{
        ...
        api.altitudeList.send(params).then(res=>{
            console.log(res)
            let tableData=[];
            if(res.isSuccess){
        ...
        return (
            <div className="forData">
                ...
                <div className="btn-type">
                    <NavLink to="/forData/environmentData/environmentDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
