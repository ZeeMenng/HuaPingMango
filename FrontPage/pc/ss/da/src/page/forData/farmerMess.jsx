...
export default class FarmerMess extends Component {
    constructor(props){
        super(props)
        this.state={
                ...
                onChange: (info) => {
                    ...
                    if (info.file.status === 'done') {
                        ...
                        if (isSuccess) {
                            ...
                            //  刷新列表
                            this.initTable(this.state.current)
                        } else {
                            message.error("上传失败！");
                        }
                ...
                },
                {
                    ...
                    render: (text, record) => (
                        <span>
                                ...
                                <NavLink to={`/forData/farmerMess/farmerMessAduit/${text}-${record.key}`}>
                                    <Icon 
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/forData/farmerMess/farmerMessAdd/${text}-${record.key}`}>
                                    <Icon 
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }} 
                                        />
    ...
    delListFn = (delArr) =>{
        ...
        api.FrameMessDelList.send(params).then(res=>{
            console.log(res)
            if(res.isSuccess){
    ...
    confirmDel=(e)=>{
        // console.log(e);
        api.FrameMessDel.send({id:e}).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    initTable=(current)=>{
        ...
        api.FrameMess.send(params).then(res=>{
            console.log(res)
            let tableData = [];
            if(!res.resultCode || Number(res.resultCode) === 950000){
                // token过期或者不可用返回登录页
            ...
            <div className="forData">
                <ul className="form-search clearfix">
                    ...
                    <li>
                        ...
                        <Input data-type="userName" onChange={this.getValue} style={{ width: '65%' }} defaultValue={userName} />
                    </li>
                    {/* <li>
                ...
                <div className="btn-type">
                    <NavLink to={`/forData/farmerMess/farmerMessAdd/1`}>
                        <Button icon="plus">单条数据填报</Button>
                    </NavLink>
