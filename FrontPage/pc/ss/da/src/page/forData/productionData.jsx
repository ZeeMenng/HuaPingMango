...
import { Select, Upload, DatePicker, Cascader, Input, Table, Button, Icon, Tooltip, Popconfirm, message } from 'antd';
...
export default class ProductionData extends Component {
    constructor(props) {
        super(props)
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daInput/importExcel',
                headers: {
            ...
            columns: [
                // { title: '采集维度', dataIndex: '0'},
                { title: '采集日期', dataIndex: '4',render: (text, row, index) => {
                    if (text) {
                      return <span>{moment(text).format('YYYY-MM-DD')}</span>;
                    }
                }},
                // { title: '区域维度',dataIndex: '2'},
                { title: '地理区域',dataIndex: '3'},
                { title: '数据日期',dataIndex: '1',render: (text, row, index) => {
                    if (text) {
                      return <span>{moment(text).format('YYYY-MM-DD')}</span>;
                    }
                }},
                { title: '名称', dataIndex: '5'},
                { title: '生产厂家',dataIndex: '6'},
                { title: '数据来源',dataIndex: '7'},
                // { title: '来源渠道',dataIndex: '8'},
                { title: '审核状态',dataIndex: '9'},
                { title: '操作', dataIndex: '10', key: 'check',
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/forData/productionData/productionDataAduit/${text}-${record.key}`}>
                                    <Icon 
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/forData/productionData/productionDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
    ...
    delListFn = (delArr) =>{
        ...
        api.inputDelList.send(params).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    confirmDel=(e)=>{
        // console.log(e);
        api.inputDel.send({id:e}).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    initAltitudeList=(current)=>{
        ...
        api.inputList.send(params).then(res=>{
            console.log(res)
            let tableData=[];
            if(res.isSuccess){
            ...
            <div className="forData">
                ...
                {/* <li>
                    ...
                    <li>
                        ...
                        <Input data-type="inputName" onChange={this.getValue} style={{ width: '65%' }} defaultValue={inputName} />
                    </li>
                    {/* <li>
                ...
                <div className="btn-type">
                    <NavLink to="/forData/productionData/productionDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
