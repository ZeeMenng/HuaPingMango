...
export default class ProcessValueData extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            propsAjax: {
                name: 'file',
                action: api.BASEURL_01 + '/extend/swagger/da/daIndustryStatistics/importExcel',
                headers: {
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/indexReport/statisticsData/StatisticsDataAduit/${text}-${record.key}`}>
                                    <Icon 
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="编辑">
                                <NavLink to={`/indexReport/statisticsData/StatisticsDataAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="form"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
     ...
     confirmDel=(e)=>{
        // console.log(e);
        api.dIStatisticsDataDel.send({id:e}).then(res=>{
            // console.log(res)
            if(res.isSuccess){
     ...
     delListFn = (delArr) =>{
        ...
        api.dIStatisticsDataDeleteList.send(params).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    initAltitudeList=(current)=>{
        ...
        api.dIStatisticsDataList.send(params).then(res=>{
            console.log(res)
            let tableData=[];
            if(res.isSuccess){
                res.data.forEach((v,i)=>{
        ...
        return (
            <div className="forData">
                ...
                <div className="btn-type">
                    <NavLink to="/indexReport/statisticsData/StatisticsDataAdd/1"><Button icon="plus">单条数据填报</Button></NavLink>
