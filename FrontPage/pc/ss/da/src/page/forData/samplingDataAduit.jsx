...
export default class SamplingDataAduit extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    initInput = (id) => {
        console.log(id)
        let prId = id.split('-')[0];
        api.samplingDataSearch.send({ id: prId }).then(res => {
            console.log(res)
            if (res.isSuccess) {
                let v = res.data;
                this.setState({
    ...
    formSaveFn = () => {
        ...
        api.samplingDataAudit.send(params).then(res => {
            console.log(res)
            if (res.isSuccess) {
    ...
    render() {
        const {
            ...
            cropTypeText, strainsText, sampleName, sampleNo, sampleDate, samplePlace, checkProject,
            samplePersonnel, remark, auditStatusText
        } = this.state
        return (
