...
export default class ProcessValueData extends Component {
    constructor(props) {
                    ...
                    render: (text, record) => (
                        <span>
                            <Tooltip placement="bottom" title="审核">
                                <NavLink to={`/publicData/priceEshop/priceEshopAduit/${text}-${record.key}`}>
                                    <Icon
                                        type="credit-card"
                             ...
                             <Tooltip placement="bottom" title="查看">
                                <NavLink to={`/publicData/priceEshop/priceEshopAdd/${text}-${record.key}`}>
                                    <Icon
                                        type="eye"
                                        style={{ padding: '0 4px', cursor: 'potainer' }}
                                    />
     ...
     confirmDel=(e)=>{
        // console.log(e);
        api.PriceEshopRemove.send({id:e}).then(res=>{
            // console.log(res)
            if(res.isSuccess){
    ...
    initAltitudeList=(current)=>{
        ...
        let params={
            "jsonData": {
                "entityRelated":{
                    ...
                    "bottomPrice" : wholePrice2,//價格下線
                    "topPrice" : wholePrice1,//價格上線
                    "productAreaText" : "",//地区文本
                    "productAreaCode" : ""//地区编码
                },
                "page":{
                    "pageIndex":current || 1,
                    "pageSize":10
        ...
        api.PriceEshopList.send(params).then(res=>{
            console.log(res)
            let tableData=[];
            if(res.isSuccess){
