...
import com.zee.dao.unity.gp.IGprRoleDomainUntDal;
...
public class GprRoleDomainGenUntBll extends BaseUntBll<GprRoleDomain> {
	...
	public ResultModel getListByRoleId(String roleId, boolean isLog) {
 	...
 	public ResultModel deleteByCompositeIdList(ArrayList<GprRoleDomain> gprRoleDomainList) {
		return deleteByCompositeIdList(gprRoleDomainList, isLogWrite);
	}

	public ResultModel deleteByCompositeIdList(ArrayList<GprRoleDomain> gprRoleDomainList, boolean isLog) {
		ResultModel result = new ResultModel();
		if (gprRoleDomainList == null || gprRoleDomainList.isEmpty()) {
		...
		try {
			result.setId(Tools.getUUID());
			result.setIncomeValue(JSONArray.fromObject(gprRoleDomainList).toString());
			result.setIncomeCount(0);
            result.setAddTime(DateUtils.getCurrentTime());
			result.setTableName(this.getClass().getSimpleName());
			result.setOperTypeCode(OperType.DELETELIST.getCode());
			...
			int i = gprRoleDomainUntDal.deleteByCompositeIdList(gprRoleDomainList);
