...
import com.zee.dao.unity.pi.IPiFileUntDal;
import com.zee.ent.custom.ResultModel;
import com.zee.ent.extend.pi.PiFile;
import com.zee.ent.parameter.pi.PiFileParameter;
 ...
 * @description CMS文章相关文件表 业务逻辑处理类，扩展自BaseUntBll<PiFile>，自动生成。
 */
public class PiFileGenUntBll extends BaseUntBll<PiFile> {

	@Autowired
	protected IPiFileUntDal piFileUntDal;


	public ResultModel updateList(PiFileParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiFileParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiFileParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiFileParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiFileParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiFileParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiFileParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiFile> list = baseUntDal.getList(map);
			PageInfo<PiFile> pageInfo = new PageInfo<PiFile>(list);
			List<PiFile> modelList = pageInfo.getList();
