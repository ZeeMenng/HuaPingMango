...
import com.zee.dao.unity.da.IDarTemplateTargetUntDal;
 ...
 * @description 模板指标中间表 业务逻辑处理类，扩展自BaseUntBll<DarTemplateTarget>，自动生成。
 */
public class DarTemplateTargetGenUntBll extends BaseUntBll<DarTemplateTarget> {

	@Autowired
	protected IDarTemplateTargetUntDal darTemplateTargetUntDal;


	public ResultModel updateList(DarTemplateTargetParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DarTemplateTargetParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DarTemplateTargetParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DarTemplateTargetParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DarTemplateTargetParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DarTemplateTargetParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DarTemplateTargetParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DarTemplateTarget> list = baseUntDal.getList(map);
			PageInfo<DarTemplateTarget> pageInfo = new PageInfo<DarTemplateTarget>(list);
			List<DarTemplateTarget> modelList = pageInfo.getList();
