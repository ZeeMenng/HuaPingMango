...
import com.zee.dao.unity.mf.IMfPrewarningValueUntDal;
 ...
 * @description 质量安全预警值设置 业务逻辑处理类，扩展自BaseUntBll<MfPrewarningValue>，自动生成。
 */
public class MfPrewarningValueGenUntBll extends BaseUntBll<MfPrewarningValue> {

	@Autowired
	protected IMfPrewarningValueUntDal mfPrewarningValueUntDal;


	public ResultModel updateList(MfPrewarningValueParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfPrewarningValueParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfPrewarningValueParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfPrewarningValueParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfPrewarningValueParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfPrewarningValueParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfPrewarningValueParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfPrewarningValue> list = baseUntDal.getList(map);
			PageInfo<MfPrewarningValue> pageInfo = new PageInfo<MfPrewarningValue>(list);
			List<MfPrewarningValue> modelList = pageInfo.getList();
