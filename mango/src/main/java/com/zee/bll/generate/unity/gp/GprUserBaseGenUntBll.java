...
public class GprUserBaseGenUntBll extends BaseUntBll<GprUserBase> {
	...
	@Autowired
	protected IGprUserBaseUntDal gprUserBaseUntDal;


	public ResultModel updateList(GprUserBaseParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GprUserBaseParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GprUserBaseParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GprUserBaseParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GprUserBaseParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GprUserBaseParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GprUserBaseParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GprUserBase> list = baseUntDal.getList(map);
			PageInfo<GprUserBase> pageInfo = new PageInfo<GprUserBase>(list);
			List<GprUserBase> modelList = pageInfo.getList();
