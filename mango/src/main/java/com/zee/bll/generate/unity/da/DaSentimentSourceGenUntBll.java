...
import com.zee.dao.unity.da.IDaSentimentSourceUntDal;
 ...
 * @description 舆情数据源表 业务逻辑处理类，扩展自BaseUntBll<DaSentimentSource>，自动生成。
 */
public class DaSentimentSourceGenUntBll extends BaseUntBll<DaSentimentSource> {

	@Autowired
	protected IDaSentimentSourceUntDal daSentimentSourceUntDal;


	public ResultModel updateList(DaSentimentSourceParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaSentimentSourceParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaSentimentSourceParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaSentimentSourceParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaSentimentSourceParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaSentimentSourceParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaSentimentSourceParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaSentimentSource> list = baseUntDal.getList(map);
			PageInfo<DaSentimentSource> pageInfo = new PageInfo<DaSentimentSource>(list);
			List<DaSentimentSource> modelList = pageInfo.getList();
