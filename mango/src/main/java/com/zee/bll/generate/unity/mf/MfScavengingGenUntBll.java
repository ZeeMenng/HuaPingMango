...
import com.zee.dao.unity.mf.IMfScavengingUntDal;
 ...
 * @updateDate 2021/1/28 16:07:18
 * @description 扫码反馈表 业务逻辑处理类，扩展自BaseUntBll<MfScavenging>，自动生成。
 */
public class MfScavengingGenUntBll extends BaseUntBll<MfScavenging> {

	@Autowired
	protected IMfScavengingUntDal mfScavengingUntDal;


	public ResultModel updateList(MfScavengingParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfScavengingParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfScavengingParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfScavengingParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfScavengingParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfScavengingParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfScavengingParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfScavenging> list = baseUntDal.getList(map);
			PageInfo<MfScavenging> pageInfo = new PageInfo<MfScavenging>(list);
			List<MfScavenging> modelList = pageInfo.getList();
