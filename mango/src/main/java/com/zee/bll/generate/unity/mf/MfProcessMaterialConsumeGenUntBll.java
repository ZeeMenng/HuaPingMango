...
import com.zee.dao.unity.mf.IMfProcessMaterialConsumeUntDal;
 ...
 * @updateDate 2021/1/29 17:00:57
 * @description 加工品原料消耗情况预测表 业务逻辑处理类，扩展自BaseUntBll<MfProcessMaterialConsume>，自动生成。
 */
public class MfProcessMaterialConsumeGenUntBll extends BaseUntBll<MfProcessMaterialConsume> {

	@Autowired
	protected IMfProcessMaterialConsumeUntDal mfProcessMaterialConsumeUntDal;


	public ResultModel updateList(MfProcessMaterialConsumeParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfProcessMaterialConsumeParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfProcessMaterialConsumeParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfProcessMaterialConsumeParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfProcessMaterialConsumeParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfProcessMaterialConsumeParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfProcessMaterialConsumeParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfProcessMaterialConsume> list = baseUntDal.getList(map);
			PageInfo<MfProcessMaterialConsume> pageInfo = new PageInfo<MfProcessMaterialConsume>(list);
			List<MfProcessMaterialConsume> modelList = pageInfo.getList();
