...
import com.zee.dao.unity.pi.IPiContentExtUntDal;
 ...
 * @description CMS内容扩展表 业务逻辑处理类，扩展自BaseUntBll<PiContentExt>，自动生成。
 */
public class PiContentExtGenUntBll extends BaseUntBll<PiContentExt> {

	@Autowired
	protected IPiContentExtUntDal piContentExtUntDal;


	public ResultModel updateList(PiContentExtParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentExtParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentExtParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentExtParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentExtParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentExtParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentExtParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentExt> list = baseUntDal.getList(map);
			PageInfo<PiContentExt> pageInfo = new PageInfo<PiContentExt>(list);
			List<PiContentExt> modelList = pageInfo.getList();
