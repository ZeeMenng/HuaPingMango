...
import com.zee.dao.unity.mf.IMfQualityInspectionRateUntDal;
import com.zee.ent.custom.ResultModel;
import com.zee.ent.extend.mf.MfQualityInspectionRate;
import com.zee.ent.parameter.mf.MfQualityInspectionRateParameter;
 ...
 * @updateDate 2021/1/29 17:00:58
 * @description 质量检测表 业务逻辑处理类，扩展自BaseUntBll<MfQualityInspectionRate>，自动生成。
 */
public class MfQualityInspectionRateGenUntBll extends BaseUntBll<MfQualityInspectionRate> {

	@Autowired
	protected IMfQualityInspectionRateUntDal mfQualityInspectionRateUntDal;


	public ResultModel updateList(MfQualityInspectionRateParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfQualityInspectionRateParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfQualityInspectionRateParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfQualityInspectionRateParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfQualityInspectionRateParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfQualityInspectionRateParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfQualityInspectionRateParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfQualityInspectionRate> list = baseUntDal.getList(map);
			PageInfo<MfQualityInspectionRate> pageInfo = new PageInfo<MfQualityInspectionRate>(list);
			List<MfQualityInspectionRate> modelList = pageInfo.getList();
