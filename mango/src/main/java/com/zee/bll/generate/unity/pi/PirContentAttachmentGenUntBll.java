...
import com.zee.dao.unity.pi.IPirContentAttachmentUntDal;
 ...
 * @updateDate 2021/1/29 17:01:00
 * @description CMS文章内容包含附件信息表。 业务逻辑处理类，扩展自BaseUntBll<PirContentAttachment>，自动生成。
 */
public class PirContentAttachmentGenUntBll extends BaseUntBll<PirContentAttachment> {

	@Autowired
	protected IPirContentAttachmentUntDal pirContentAttachmentUntDal;


	public ResultModel updateList(PirContentAttachmentParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirContentAttachmentParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirContentAttachmentParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirContentAttachmentParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirContentAttachmentParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirContentAttachmentParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirContentAttachmentParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirContentAttachment> list = baseUntDal.getList(map);
			PageInfo<PirContentAttachment> pageInfo = new PageInfo<PirContentAttachment>(list);
			List<PirContentAttachment> modelList = pageInfo.getList();
