...
import com.zee.dao.unity.da.IDaCommonFieldUntDal;
import com.zee.ent.custom.ResultModel;
import com.zee.ent.extend.da.DaCommonField;
import com.zee.ent.parameter.da.DaCommonFieldParameter;
 ...
 * @description 通用字段表 业务逻辑处理类，扩展自BaseUntBll<DaCommonField>，自动生成。
 */
public class DaCommonFieldGenUntBll extends BaseUntBll<DaCommonField> {

	@Autowired
	protected IDaCommonFieldUntDal daCommonFieldUntDal;


	public ResultModel updateList(DaCommonFieldParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaCommonFieldParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaCommonFieldParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaCommonFieldParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaCommonFieldParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaCommonFieldParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaCommonFieldParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaCommonField> list = baseUntDal.getList(map);
			PageInfo<DaCommonField> pageInfo = new PageInfo<DaCommonField>(list);
			List<DaCommonField> modelList = pageInfo.getList();
