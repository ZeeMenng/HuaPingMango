...
import com.zee.dao.unity.pi.IPiExpertUntDal;
 ...
 * @description 专家表 业务逻辑处理类，扩展自BaseUntBll<PiExpert>，自动生成。
 */
public class PiExpertGenUntBll extends BaseUntBll<PiExpert> {

	@Autowired
	protected IPiExpertUntDal piExpertUntDal;


	public ResultModel updateList(PiExpertParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiExpertParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiExpertParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiExpertParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiExpertParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiExpertParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiExpertParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiExpert> list = baseUntDal.getList(map);
			PageInfo<PiExpert> pageInfo = new PageInfo<PiExpert>(list);
			List<PiExpert> modelList = pageInfo.getList();
