...
import com.zee.dao.unity.da.IDaTemplateDataUntDal;
 ...
 * @description 模板数据表 业务逻辑处理类，扩展自BaseUntBll<DaTemplateData>，自动生成。
 */
public class DaTemplateDataGenUntBll extends BaseUntBll<DaTemplateData> {

	@Autowired
	protected IDaTemplateDataUntDal daTemplateDataUntDal;


	public ResultModel updateList(DaTemplateDataParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaTemplateDataParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaTemplateDataParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaTemplateDataParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaTemplateDataParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaTemplateDataParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaTemplateDataParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaTemplateData> list = baseUntDal.getList(map);
			PageInfo<DaTemplateData> pageInfo = new PageInfo<DaTemplateData>(list);
			List<DaTemplateData> modelList = pageInfo.getList();
