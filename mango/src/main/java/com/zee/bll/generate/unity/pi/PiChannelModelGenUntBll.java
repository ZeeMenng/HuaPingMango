...
import com.zee.dao.unity.pi.IPiChannelModelUntDal;
 ...
 * @updateDate 2021/1/29 17:01:04
 * @description CMS栏目可选内容模型表 业务逻辑处理类，扩展自BaseUntBll<PiChannelModel>，自动生成。
 */
public class PiChannelModelGenUntBll extends BaseUntBll<PiChannelModel> {

	@Autowired
	protected IPiChannelModelUntDal piChannelModelUntDal;


	public ResultModel updateList(PiChannelModelParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiChannelModelParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiChannelModelParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiChannelModelParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiChannelModelParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiChannelModelParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiChannelModelParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiChannelModel> list = baseUntDal.getList(map);
			PageInfo<PiChannelModel> pageInfo = new PageInfo<PiChannelModel>(list);
			List<PiChannelModel> modelList = pageInfo.getList();
