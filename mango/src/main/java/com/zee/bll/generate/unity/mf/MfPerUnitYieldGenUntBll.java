...
import com.zee.dao.unity.mf.IMfPerUnitYieldUntDal;
 ...
 * @description 单产预测建模表 业务逻辑处理类，扩展自BaseUntBll<MfPerUnitYield>，自动生成。
 */
public class MfPerUnitYieldGenUntBll extends BaseUntBll<MfPerUnitYield> {

	@Autowired
	protected IMfPerUnitYieldUntDal mfPerUnitYieldUntDal;


	public ResultModel updateList(MfPerUnitYieldParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfPerUnitYieldParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfPerUnitYieldParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfPerUnitYieldParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfPerUnitYieldParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfPerUnitYieldParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfPerUnitYieldParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfPerUnitYield> list = baseUntDal.getList(map);
			PageInfo<MfPerUnitYield> pageInfo = new PageInfo<MfPerUnitYield>(list);
			List<MfPerUnitYield> modelList = pageInfo.getList();
