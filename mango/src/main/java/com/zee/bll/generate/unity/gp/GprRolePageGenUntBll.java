...
import com.zee.dao.unity.gp.IGprRolePageUntDal;
...
public class GprRolePageGenUntBll extends BaseUntBll<GprRolePage> {
	...
	public ResultModel deleteByPageIdList(ArrayList<String> pageIdList, boolean isLog) {
		...
		try {
			...
			int i = gprRolePageUntDal.deleteByPageIdList(pageIdList);
	...
	public ResultModel deleteByRoleIdList(ArrayList<String> roleIdList, boolean isLog) {
		...
		try {
			...
			int i = gprRolePageUntDal.deleteByRoleIdList(roleIdList);
	...
	public ResultModel getListByRoleId(String roleId, boolean isLog) {
 	...
 	public ResultModel deleteByCompositeIdList(ArrayList<GprRolePage> gprRolePageList) {
		return deleteByCompositeIdList(gprRolePageList, isLogWrite);
	}

	public ResultModel deleteByCompositeIdList(ArrayList<GprRolePage> gprRolePageList, boolean isLog) {
		ResultModel result = new ResultModel();
		if (gprRolePageList == null || gprRolePageList.isEmpty()) {
		...
		try {
			result.setId(Tools.getUUID());
			result.setIncomeValue(JSONArray.fromObject(gprRolePageList).toString());
			result.setIncomeCount(0);
            result.setAddTime(DateUtils.getCurrentTime());
			result.setTableName(this.getClass().getSimpleName());
			result.setOperTypeCode(OperType.DELETELIST.getCode());
			...
			int i = gprRolePageUntDal.deleteByCompositeIdList(gprRolePageList);
