...
import com.zee.dao.unity.da.IDaSentimentContentUntDal;
 ...
 * @updateDate 2021/1/29 17:00:41
 * @description 舆情文章内容表 业务逻辑处理类，扩展自BaseUntBll<DaSentimentContent>，自动生成。
 */
public class DaSentimentContentGenUntBll extends BaseUntBll<DaSentimentContent> {

	@Autowired
	protected IDaSentimentContentUntDal daSentimentContentUntDal;


	public ResultModel updateList(DaSentimentContentParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaSentimentContentParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaSentimentContentParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaSentimentContentParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaSentimentContentParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaSentimentContentParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaSentimentContentParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaSentimentContent> list = baseUntDal.getList(map);
			PageInfo<DaSentimentContent> pageInfo = new PageInfo<DaSentimentContent>(list);
			List<DaSentimentContent> modelList = pageInfo.getList();
