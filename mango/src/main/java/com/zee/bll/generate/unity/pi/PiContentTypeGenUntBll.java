...
import com.zee.dao.unity.pi.IPiContentTypeUntDal;
 ...
 * @updateDate 2021/1/29 17:01:08
 * @description CMS内容类型表 业务逻辑处理类，扩展自BaseUntBll<PiContentType>，自动生成。
 */
public class PiContentTypeGenUntBll extends BaseUntBll<PiContentType> {

	@Autowired
	protected IPiContentTypeUntDal piContentTypeUntDal;


	public ResultModel updateList(PiContentTypeParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentTypeParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentTypeParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentTypeParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentTypeParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentTypeParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentTypeParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentType> list = baseUntDal.getList(map);
			PageInfo<PiContentType> pageInfo = new PageInfo<PiContentType>(list);
			List<PiContentType> modelList = pageInfo.getList();
