...
import com.zee.dao.unity.pi.IPirInteractionImageUntDal;
 ...
 * @updateDate 2021/1/29 17:01:02
 * @description pir_interaction_image 业务逻辑处理类，扩展自BaseUntBll<PirInteractionImage>，自动生成。
 */
public class PirInteractionImageGenUntBll extends BaseUntBll<PirInteractionImage> {

	@Autowired
	protected IPirInteractionImageUntDal pirInteractionImageUntDal;


	public ResultModel updateList(PirInteractionImageParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirInteractionImageParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirInteractionImageParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirInteractionImageParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirInteractionImageParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirInteractionImageParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirInteractionImageParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirInteractionImage> list = baseUntDal.getList(map);
			PageInfo<PirInteractionImage> pageInfo = new PageInfo<PirInteractionImage>(list);
			List<PirInteractionImage> modelList = pageInfo.getList();
