...
import com.zee.dao.unity.da.IDaOnlineRetailersUntDal;
 ...
 * @updateDate 2021/1/28 16:06:57
 * @description 芒果电商分部表 业务逻辑处理类，扩展自BaseUntBll<DaOnlineRetailers>，自动生成。
 */
public class DaOnlineRetailersGenUntBll extends BaseUntBll<DaOnlineRetailers> {

	@Autowired
	protected IDaOnlineRetailersUntDal daOnlineRetailersUntDal;


	public ResultModel updateList(DaOnlineRetailersParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaOnlineRetailersParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaOnlineRetailersParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaOnlineRetailersParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaOnlineRetailersParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaOnlineRetailersParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaOnlineRetailersParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaOnlineRetailers> list = baseUntDal.getList(map);
			PageInfo<DaOnlineRetailers> pageInfo = new PageInfo<DaOnlineRetailers>(list);
			List<DaOnlineRetailers> modelList = pageInfo.getList();
