...
import com.zee.dao.unity.pi.IPiInteractionUntDal;
 ...
 * @description pi_interaction 业务逻辑处理类，扩展自BaseUntBll<PiInteraction>，自动生成。
 */
public class PiInteractionGenUntBll extends BaseUntBll<PiInteraction> {

	@Autowired
	protected IPiInteractionUntDal piInteractionUntDal;


	public ResultModel updateList(PiInteractionParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiInteractionParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiInteractionParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiInteractionParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiInteractionParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiInteractionParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiInteractionParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiInteraction> list = baseUntDal.getList(map);
			PageInfo<PiInteraction> pageInfo = new PageInfo<PiInteraction>(list);
			List<PiInteraction> modelList = pageInfo.getList();
