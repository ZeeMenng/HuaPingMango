...
import com.zee.dao.unity.mf.IMfProcessVolumeUntDal;
 ...
 * @description mf_process_volume 业务逻辑处理类，扩展自BaseUntBll<MfProcessVolume>，自动生成。
 */
public class MfProcessVolumeGenUntBll extends BaseUntBll<MfProcessVolume> {

	@Autowired
	protected IMfProcessVolumeUntDal mfProcessVolumeUntDal;


	public ResultModel updateList(MfProcessVolumeParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfProcessVolumeParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfProcessVolumeParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfProcessVolumeParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfProcessVolumeParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfProcessVolumeParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfProcessVolumeParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfProcessVolume> list = baseUntDal.getList(map);
			PageInfo<MfProcessVolume> pageInfo = new PageInfo<MfProcessVolume>(list);
			List<MfProcessVolume> modelList = pageInfo.getList();
