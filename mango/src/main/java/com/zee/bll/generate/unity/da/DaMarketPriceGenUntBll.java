 ...
 * @description 市场价格数据表 业务逻辑处理类，扩展自BaseUntBll<DaMarketPrice>，自动生成。
 */
public class DaMarketPriceGenUntBll extends BaseUntBll<DaMarketPrice> {

	@Autowired
	protected IDaMarketPriceUntDal daMarketPriceUntDal;


	public ResultModel updateList(DaMarketPriceParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaMarketPriceParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaMarketPriceParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaMarketPriceParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaMarketPriceParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaMarketPriceParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaMarketPriceParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaMarketPrice> list = baseUntDal.getList(map);
			PageInfo<DaMarketPrice> pageInfo = new PageInfo<DaMarketPrice>(list);
			List<DaMarketPrice> modelList = pageInfo.getList();
