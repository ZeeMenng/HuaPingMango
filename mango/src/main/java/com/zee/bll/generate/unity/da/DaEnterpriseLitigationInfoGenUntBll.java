...
import com.zee.dao.unity.da.IDaEnterpriseLitigationInfoUntDal;
import com.zee.ent.custom.ResultModel;
import com.zee.ent.extend.da.DaEnterpriseLitigationInfo;
import com.zee.ent.parameter.da.DaEnterpriseLitigationInfoParameter;
 ...
 * @updateDate 2021/1/29 17:00:35
 * @description 企业诉讼表 业务逻辑处理类，扩展自BaseUntBll<DaEnterpriseLitigationInfo>，自动生成。
 */
public class DaEnterpriseLitigationInfoGenUntBll extends BaseUntBll<DaEnterpriseLitigationInfo> {

	@Autowired
	protected IDaEnterpriseLitigationInfoUntDal daEnterpriseLitigationInfoUntDal;


	public ResultModel updateList(DaEnterpriseLitigationInfoParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaEnterpriseLitigationInfoParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaEnterpriseLitigationInfoParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaEnterpriseLitigationInfoParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaEnterpriseLitigationInfoParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaEnterpriseLitigationInfoParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaEnterpriseLitigationInfoParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaEnterpriseLitigationInfo> list = baseUntDal.getList(map);
			PageInfo<DaEnterpriseLitigationInfo> pageInfo = new PageInfo<DaEnterpriseLitigationInfo>(list);
			List<DaEnterpriseLitigationInfo> modelList = pageInfo.getList();
