...
import com.zee.dao.unity.mf.IMfQualityUntDal;
 ...
 * @description 质量安全综合指数表,建模 业务逻辑处理类，扩展自BaseUntBll<MfQuality>，自动生成。
 */
public class MfQualityGenUntBll extends BaseUntBll<MfQuality> {

	@Autowired
	protected IMfQualityUntDal mfQualityUntDal;


	public ResultModel updateList(MfQualityParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfQualityParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfQualityParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfQualityParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfQualityParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfQualityParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfQualityParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfQuality> list = baseUntDal.getList(map);
			PageInfo<MfQuality> pageInfo = new PageInfo<MfQuality>(list);
			List<MfQuality> modelList = pageInfo.getList();
