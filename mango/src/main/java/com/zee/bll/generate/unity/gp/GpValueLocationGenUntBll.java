...
import com.zee.dao.unity.gp.IGpValueLocationUntDal;
...
public class GpValueLocationGenUntBll extends BaseUntBll<GpValueLocation> {
	...
	@Autowired
	protected IGpValueLocationUntDal gpValueLocationUntDal;


	public ResultModel updateList(GpValueLocationParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpValueLocationParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpValueLocationParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpValueLocationParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpValueLocationParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpValueLocationParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpValueLocationParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpValueLocation> list = baseUntDal.getList(map);
			PageInfo<GpValueLocation> pageInfo = new PageInfo<GpValueLocation>(list);
			List<GpValueLocation> modelList = pageInfo.getList();
