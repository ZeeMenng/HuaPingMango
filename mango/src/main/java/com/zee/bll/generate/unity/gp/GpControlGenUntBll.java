...
import com.zee.dao.unity.gp.IGpControlUntDal;
 ...
 * @updateDate 2021/1/28 16:07:08
 * @description 系统控件。 业务逻辑处理类，扩展自BaseUntBll<GpControl>，自动生成。
 */
public class GpControlGenUntBll extends BaseUntBll<GpControl> {

	@Autowired
	protected IGpControlUntDal gpControlUntDal;


	public ResultModel updateList(GpControlParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpControlParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpControlParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpControlParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpControlParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpControlParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpControlParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpControl> list = baseUntDal.getList(map);
			PageInfo<GpControl> pageInfo = new PageInfo<GpControl>(list);
			List<GpControl> modelList = pageInfo.getList();
	...
	public ResultModel deleteByDomainId(String domainId, boolean isLog) {
		...
		try {
			...
			int i = gpControlUntDal.deleteByDomainId( domainId);
	...
	public ResultModel deleteByDomainIdList(ArrayList<String> domainIdList, boolean isLog) {
		...
		try {
			...
			int i = gpControlUntDal.deleteByDomainIdList(domainIdList);
	...
	public ResultModel getListByDomainId(String domainId, boolean isLog) {
		...
		try {
			...
			List<GpControl> modelList = gpControlUntDal.getListByDomainId(domainId);
	...
	public ResultModel deleteByPageId(String pageId, boolean isLog) {
		...
		try {
			...
			int i = gpControlUntDal.deleteByPageId( pageId);
 	...
 	public ResultModel deleteByPageIdList(ArrayList<String> pageIdList) {
		return deleteByPageIdList(pageIdList, isLogRead);
	}

	public ResultModel deleteByPageIdList(ArrayList<String> pageIdList, boolean isLog) {
		ResultModel result = new ResultModel();
		try {
			result.setAddTime(DateUtils.getCurrentTime());
			result.setId(Tools.getUUID());
			result.setIncomeValue(JSONArray.fromObject( pageIdList).toString());
			result.setObjectId("");
			result.setTableName(this.getClass().getSimpleName());
			result.setOperTypeCode(OperType.DELETELIST.getCode());
			result.setOperTypeText(OperType.DELETELIST.getText());
			...
			int i = gpControlUntDal.deleteByPageIdList(pageIdList);
	...
	public ResultModel getListByPageId(String pageId, boolean isLog) {
		...
		try {
			...
			List<GpControl> modelList = gpControlUntDal.getListByPageId(pageId);
