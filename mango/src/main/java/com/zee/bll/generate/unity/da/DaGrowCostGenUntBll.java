...
import com.zee.dao.unity.da.IDaGrowCostUntDal;
 ...
 * @description 种植成本 业务逻辑处理类，扩展自BaseUntBll<DaGrowCost>，自动生成。
 */
public class DaGrowCostGenUntBll extends BaseUntBll<DaGrowCost> {

	@Autowired
	protected IDaGrowCostUntDal daGrowCostUntDal;


	public ResultModel updateList(DaGrowCostParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaGrowCostParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaGrowCostParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaGrowCostParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaGrowCostParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaGrowCostParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaGrowCostParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaGrowCost> list = baseUntDal.getList(map);
			PageInfo<DaGrowCost> pageInfo = new PageInfo<DaGrowCost>(list);
			List<DaGrowCost> modelList = pageInfo.getList();
