...
import com.zee.dao.unity.da.IDaGrowYieldUntDal;
 ...
 * @description 种植数据表 业务逻辑处理类，扩展自BaseUntBll<DaGrowYield>，自动生成。
 */
public class DaGrowYieldGenUntBll extends BaseUntBll<DaGrowYield> {

	@Autowired
	protected IDaGrowYieldUntDal daGrowYieldUntDal;


	public ResultModel updateList(DaGrowYieldParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaGrowYieldParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaGrowYieldParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaGrowYieldParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaGrowYieldParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaGrowYieldParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaGrowYieldParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaGrowYield> list = baseUntDal.getList(map);
			PageInfo<DaGrowYield> pageInfo = new PageInfo<DaGrowYield>(list);
			List<DaGrowYield> modelList = pageInfo.getList();
