...
import com.jusfoun.bll.generate.split.gp.GprUserOrganizationGenSplBll;
 ...
 * @description 用户所属组织机构。 业务逻辑处理类，扩展自BaseSplBll<GprUserOrganization>，可手动更改。
 */
@Service("gprUserOrganizationSplBll")
public class GprUserOrganizationSplBll extends GprUserOrganizationGenSplBll {
	...
	public ResultModel deleteByUserId(String userId, boolean isLog) {
		...
		try {
			...
			result.setRemark("根据用户ID列表，删除用户组织机构中间表。");

			int i = gprUserOrganizationSplDal.deleteByUserId(userId);
	...
	public ResultModel deleteByUserIdList(ArrayList<String> userIdList, boolean isLog) {
		...
		try {
			...
			result.setRemark("根据用户ID列表，批量删除用户组织机构中间表。");

			int i = gprUserOrganizationSplDal.deleteByUserIdList(userIdList);
