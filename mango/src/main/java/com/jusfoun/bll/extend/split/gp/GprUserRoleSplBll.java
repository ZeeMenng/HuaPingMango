...
import com.jusfoun.bll.generate.split.gp.GprUserRoleGenSplBll;
 ...
 * @description 用户拥有的角色。 业务逻辑处理类，扩展自BaseSplBll<GprUserRole>，可手动更改。
 */
@Service("gprUserRoleSplBll")
public class GprUserRoleSplBll extends GprUserRoleGenSplBll {
	...
	public ResultModel deleteByUserId(String userId, boolean isLog) {
		...
		try {
			...
			int i = gprUserRoleSplDal.deleteByUserId(userId);
	...
	public ResultModel deleteByUserIdList(ArrayList<String> userIdList, boolean isLog) {
		...
		try {
			...
			result.setRemark("根据用户ID列表，批量删除用户角色中间表。");

			int i = gprUserRoleSplDal.deleteByUserIdList(userIdList);
