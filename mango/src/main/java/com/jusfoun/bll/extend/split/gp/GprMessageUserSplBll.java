...
import com.jusfoun.bll.generate.split.gp.GprMessageUserGenSplBll;
 ...
 * @description 消息接收者及消息读取状态。 业务逻辑处理类，扩展自BaseSplBll<GprMessageUser>，可手动更改。
 */
...
public class GprMessageUserSplBll extends GprMessageUserGenSplBll {
	...
	public ResultModel deleteByUserId(String userId, boolean isLog) {
		...
		try {
			...
			result.setRemark("根据用户ID，删除用户消息中间表。");

			int i = gprMessageUserSplDal.deleteByUserId(userId);
	...
	public ResultModel deleteByUserIdList(ArrayList<String> userIdList, boolean isLog) {
		...
		try {
			...
			result.setRemark("根据用户ID列表，批量删除用户消息中间表。");

			int i = gprMessageUserSplDal.deleteByUserIdList(userIdList);
