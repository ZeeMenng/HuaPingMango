...
import com.jusfoun.dao.unity.pi.IPiChannelTxtUntDal;
 ...
 * @description CMS栏目文本表 业务逻辑处理类，扩展自BaseUntBll<PiChannelTxt>，自动生成。
 */
public class PiChannelTxtGenUntBll extends BaseUntBll<PiChannelTxt> {

	@Autowired
	protected IPiChannelTxtUntDal piChannelTxtUntDal;


	public ResultModel updateList(PiChannelTxtParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiChannelTxtParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiChannelTxtParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiChannelTxtParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiChannelTxtParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiChannelTxtParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiChannelTxtParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiChannelTxt> list = baseUntDal.getList(map);
			PageInfo<PiChannelTxt> pageInfo = new PageInfo<PiChannelTxt>(list);
			List<PiChannelTxt> modelList = pageInfo.getList();
