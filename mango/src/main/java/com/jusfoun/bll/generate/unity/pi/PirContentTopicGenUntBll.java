...
import com.jusfoun.dao.unity.pi.IPirContentTopicUntDal;
 ...
 * @description CMS专题内容关联表 业务逻辑处理类，扩展自BaseUntBll<PirContentTopic>，自动生成。
 */
public class PirContentTopicGenUntBll extends BaseUntBll<PirContentTopic> {

	@Autowired
	protected IPirContentTopicUntDal pirContentTopicUntDal;


	public ResultModel updateList(PirContentTopicParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirContentTopicParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirContentTopicParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirContentTopicParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirContentTopicParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirContentTopicParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirContentTopicParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirContentTopic> list = baseUntDal.getList(map);
			PageInfo<PirContentTopic> pageInfo = new PageInfo<PirContentTopic>(list);
			List<PirContentTopic> modelList = pageInfo.getList();
