...
import com.jusfoun.dao.unity.pi.IPirContentTagUntDal;
 ...
 * @description CMS内容标签关联表 业务逻辑处理类，扩展自BaseUntBll<PirContentTag>，自动生成。
 */
public class PirContentTagGenUntBll extends BaseUntBll<PirContentTag> {

	@Autowired
	protected IPirContentTagUntDal pirContentTagUntDal;


	public ResultModel updateList(PirContentTagParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirContentTagParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirContentTagParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirContentTagParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirContentTagParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirContentTagParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirContentTagParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirContentTag> list = baseUntDal.getList(map);
			PageInfo<PirContentTag> pageInfo = new PageInfo<PirContentTag>(list);
			List<PirContentTag> modelList = pageInfo.getList();
