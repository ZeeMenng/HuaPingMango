...
import com.jusfoun.dao.unity.gp.IGpPageUntDal;
 ...
 * @description 系统页面。 业务逻辑处理类，扩展自BaseUntBll<GpPage>，自动生成。
 */
public class GpPageGenUntBll extends BaseUntBll<GpPage> {

	@Autowired
	protected IGpPageUntDal gpPageUntDal;


	public ResultModel updateList(GpPageParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpPageParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpPageParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpPageParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpPageParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpPageParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpPageParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpPage> list = baseUntDal.getList(map);
			PageInfo<GpPage> pageInfo = new PageInfo<GpPage>(list);
			List<GpPage> modelList = pageInfo.getList();
	...
	public ResultModel deleteByDomainId(String domainId, boolean isLog) {
		...
		try {
			...
			int i = gpPageUntDal.deleteByDomainId( domainId);
	...
	public ResultModel deleteByDomainIdList(ArrayList<String> domainIdList, boolean isLog) {
		...
		try {
			...
			int i = gpPageUntDal.deleteByDomainIdList(domainIdList);
	...
	public ResultModel getListByDomainId(String domainId, boolean isLog) {
		...
		try {
			...
			List<GpPage> modelList = gpPageUntDal.getListByDomainId(domainId);
