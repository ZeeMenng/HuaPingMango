...
import com.zee.dao.unity.mf.IMfHpFluctUntDal;
import com.zee.ent.custom.ResultModel;
import com.zee.ent.extend.mf.MfHpFluct;
import com.zee.ent.parameter.mf.MfHpFluctParameter;
 ...
 * @description 批发价格周期性波动规律分析 业务逻辑处理类，扩展自BaseUntBll<MfHpFluct>，自动生成。
 */
public class MfHpFluctGenUntBll extends BaseUntBll<MfHpFluct> {

	@Autowired
	protected IMfHpFluctUntDal mfHpFluctUntDal;


	public ResultModel updateList(MfHpFluctParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfHpFluctParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfHpFluctParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfHpFluctParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfHpFluctParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfHpFluctParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfHpFluctParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfHpFluct> list = baseUntDal.getList(map);
			PageInfo<MfHpFluct> pageInfo = new PageInfo<MfHpFluct>(list);
			List<MfHpFluct> modelList = pageInfo.getList();
