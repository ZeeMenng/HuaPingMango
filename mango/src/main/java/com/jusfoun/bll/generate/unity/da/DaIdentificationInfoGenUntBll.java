...
import com.zee.dao.unity.da.IDaIdentificationInfoUntDal;
 ...
 * @updateDate 2021/1/28 16:06:53
 * @description 三品一标认证表 业务逻辑处理类，扩展自BaseUntBll<DaIdentificationInfo>，自动生成。
 */
public class DaIdentificationInfoGenUntBll extends BaseUntBll<DaIdentificationInfo> {

	@Autowired
	protected IDaIdentificationInfoUntDal daIdentificationInfoUntDal;


	public ResultModel updateList(DaIdentificationInfoParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaIdentificationInfoParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaIdentificationInfoParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaIdentificationInfoParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaIdentificationInfoParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaIdentificationInfoParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaIdentificationInfoParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaIdentificationInfo> list = baseUntDal.getList(map);
			PageInfo<DaIdentificationInfo> pageInfo = new PageInfo<DaIdentificationInfo>(list);
			List<DaIdentificationInfo> modelList = pageInfo.getList();
