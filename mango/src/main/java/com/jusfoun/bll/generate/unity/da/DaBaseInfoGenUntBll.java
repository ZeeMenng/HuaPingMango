...
import com.jusfoun.dao.unity.da.IDaBaseInfoUntDal;
 ...
 * @description 基地信息表 业务逻辑处理类，扩展自BaseUntBll<DaBaseInfo>，自动生成。
 */
public class DaBaseInfoGenUntBll extends BaseUntBll<DaBaseInfo> {

	@Autowired
	protected IDaBaseInfoUntDal daBaseInfoUntDal;


	public ResultModel updateList(DaBaseInfoParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaBaseInfoParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaBaseInfoParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaBaseInfoParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaBaseInfoParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaBaseInfoParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaBaseInfoParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaBaseInfo> list = baseUntDal.getList(map);
			PageInfo<DaBaseInfo> pageInfo = new PageInfo<DaBaseInfo>(list);
			List<DaBaseInfo> modelList = pageInfo.getList();
