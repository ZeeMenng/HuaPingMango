...
import com.zee.dao.unity.da.IDaSentimentRuleUntDal;
 ...
 * @description 舆情采集规则表 业务逻辑处理类，扩展自BaseUntBll<DaSentimentRule>，自动生成。
 */
public class DaSentimentRuleGenUntBll extends BaseUntBll<DaSentimentRule> {

	@Autowired
	protected IDaSentimentRuleUntDal daSentimentRuleUntDal;


	public ResultModel updateList(DaSentimentRuleParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaSentimentRuleParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaSentimentRuleParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaSentimentRuleParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaSentimentRuleParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaSentimentRuleParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaSentimentRuleParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaSentimentRule> list = baseUntDal.getList(map);
			PageInfo<DaSentimentRule> pageInfo = new PageInfo<DaSentimentRule>(list);
			List<DaSentimentRule> modelList = pageInfo.getList();
