...
import com.jusfoun.dao.unity.da.IDaCorpDisasterUntDal;
 ...
 * @description 作物受灾数据表 业务逻辑处理类，扩展自BaseUntBll<DaCorpDisaster>，自动生成。
 */
public class DaCorpDisasterGenUntBll extends BaseUntBll<DaCorpDisaster> {

	@Autowired
	protected IDaCorpDisasterUntDal daCorpDisasterUntDal;


	public ResultModel updateList(DaCorpDisasterParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaCorpDisasterParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaCorpDisasterParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaCorpDisasterParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaCorpDisasterParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaCorpDisasterParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaCorpDisasterParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaCorpDisaster> list = baseUntDal.getList(map);
			PageInfo<DaCorpDisaster> pageInfo = new PageInfo<DaCorpDisaster>(list);
			List<DaCorpDisaster> modelList = pageInfo.getList();
