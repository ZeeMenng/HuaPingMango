...
import com.jusfoun.dao.unity.pi.IPiChannelUntDal;
 ...
 * @description CMS栏目表 业务逻辑处理类，扩展自BaseUntBll<PiChannel>，自动生成。
 */
public class PiChannelGenUntBll extends BaseUntBll<PiChannel> {

	@Autowired
	protected IPiChannelUntDal piChannelUntDal;


	public ResultModel updateList(PiChannelParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiChannelParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiChannelParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiChannelParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiChannelParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiChannelParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiChannelParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiChannel> list = baseUntDal.getList(map);
			PageInfo<PiChannel> pageInfo = new PageInfo<PiChannel>(list);
			List<PiChannel> modelList = pageInfo.getList();
