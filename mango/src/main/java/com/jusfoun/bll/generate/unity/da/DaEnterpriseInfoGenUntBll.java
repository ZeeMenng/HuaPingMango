...
import com.zee.dao.unity.da.IDaEnterpriseInfoUntDal;
 ...
 * @description da_enterprise_info 业务逻辑处理类，扩展自BaseUntBll<DaEnterpriseInfo>，自动生成。
 */
public class DaEnterpriseInfoGenUntBll extends BaseUntBll<DaEnterpriseInfo> {

	@Autowired
	protected IDaEnterpriseInfoUntDal daEnterpriseInfoUntDal;


	public ResultModel updateList(DaEnterpriseInfoParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaEnterpriseInfoParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaEnterpriseInfoParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaEnterpriseInfoParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaEnterpriseInfoParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaEnterpriseInfoParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaEnterpriseInfoParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaEnterpriseInfo> list = baseUntDal.getList(map);
			PageInfo<DaEnterpriseInfo> pageInfo = new PageInfo<DaEnterpriseInfo>(list);
			List<DaEnterpriseInfo> modelList = pageInfo.getList();
