...
import com.zee.dao.unity.mf.IMfInputTypeUntDal;
 ...
 * @description 投入品种类监管 业务逻辑处理类，扩展自BaseUntBll<MfInputType>，自动生成。
 */
public class MfInputTypeGenUntBll extends BaseUntBll<MfInputType> {

	@Autowired
	protected IMfInputTypeUntDal mfInputTypeUntDal;


	public ResultModel updateList(MfInputTypeParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfInputTypeParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfInputTypeParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfInputTypeParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfInputTypeParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfInputTypeParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfInputTypeParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfInputType> list = baseUntDal.getList(map);
			PageInfo<MfInputType> pageInfo = new PageInfo<MfInputType>(list);
			List<MfInputType> modelList = pageInfo.getList();
