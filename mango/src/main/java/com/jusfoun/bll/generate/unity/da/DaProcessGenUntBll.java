...
import com.jusfoun.dao.unity.da.IDaProcessUntDal;
 ...
 * @description 加工品数据表 业务逻辑处理类，扩展自BaseUntBll<DaProcess>，自动生成。
 */
public class DaProcessGenUntBll extends BaseUntBll<DaProcess> {

	@Autowired
	protected IDaProcessUntDal daProcessUntDal;


	public ResultModel updateList(DaProcessParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaProcessParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaProcessParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaProcessParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaProcessParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaProcessParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaProcessParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaProcess> list = baseUntDal.getList(map);
			PageInfo<DaProcess> pageInfo = new PageInfo<DaProcess>(list);
			List<DaProcess> modelList = pageInfo.getList();
