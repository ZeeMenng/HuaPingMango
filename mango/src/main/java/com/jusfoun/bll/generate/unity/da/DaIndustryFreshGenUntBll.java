...
import com.jusfoun.dao.unity.da.IDaIndustryFreshUntDal;
 ...
 * @description 鲜果产值数据表 业务逻辑处理类，扩展自BaseUntBll<DaIndustryFresh>，自动生成。
 */
public class DaIndustryFreshGenUntBll extends BaseUntBll<DaIndustryFresh> {

	@Autowired
	protected IDaIndustryFreshUntDal daIndustryFreshUntDal;


	public ResultModel updateList(DaIndustryFreshParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaIndustryFreshParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaIndustryFreshParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaIndustryFreshParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaIndustryFreshParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaIndustryFreshParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaIndustryFreshParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaIndustryFresh> list = baseUntDal.getList(map);
			PageInfo<DaIndustryFresh> pageInfo = new PageInfo<DaIndustryFresh>(list);
			List<DaIndustryFresh> modelList = pageInfo.getList();
