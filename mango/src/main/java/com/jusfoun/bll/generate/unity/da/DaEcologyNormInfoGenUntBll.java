...
import com.zee.dao.unity.da.IDaEcologyNormInfoUntDal;
 ...
 * @description 种植生态指标表 业务逻辑处理类，扩展自BaseUntBll<DaEcologyNormInfo>，自动生成。
 */
public class DaEcologyNormInfoGenUntBll extends BaseUntBll<DaEcologyNormInfo> {

	@Autowired
	protected IDaEcologyNormInfoUntDal daEcologyNormInfoUntDal;


	public ResultModel updateList(DaEcologyNormInfoParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaEcologyNormInfoParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaEcologyNormInfoParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaEcologyNormInfoParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaEcologyNormInfoParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaEcologyNormInfoParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaEcologyNormInfoParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaEcologyNormInfo> list = baseUntDal.getList(map);
			PageInfo<DaEcologyNormInfo> pageInfo = new PageInfo<DaEcologyNormInfo>(list);
			List<DaEcologyNormInfo> modelList = pageInfo.getList();
