...
import com.jusfoun.dao.unity.pi.IPiProductRecommendUntDal;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.extend.pi.PiProductRecommend;
import com.jusfoun.ent.parameter.pi.PiProductRecommendParameter;
 ...
 * @description 企业推介产品表 业务逻辑处理类，扩展自BaseUntBll<PiProductRecommend>，自动生成。
 */
public class PiProductRecommendGenUntBll extends BaseUntBll<PiProductRecommend> {

	@Autowired
	protected IPiProductRecommendUntDal piProductRecommendUntDal;


	public ResultModel updateList(PiProductRecommendParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiProductRecommendParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiProductRecommendParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiProductRecommendParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiProductRecommendParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiProductRecommendParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiProductRecommendParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiProductRecommend> list = baseUntDal.getList(map);
			PageInfo<PiProductRecommend> pageInfo = new PageInfo<PiProductRecommend>(list);
			List<PiProductRecommend> modelList = pageInfo.getList();
