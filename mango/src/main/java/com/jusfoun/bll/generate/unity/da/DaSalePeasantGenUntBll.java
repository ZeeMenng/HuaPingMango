...
import com.jusfoun.dao.unity.da.IDaSalePeasantUntDal;
 ...
 * @description 农户销售数据表 业务逻辑处理类，扩展自BaseUntBll<DaSalePeasant>，自动生成。
 */
public class DaSalePeasantGenUntBll extends BaseUntBll<DaSalePeasant> {

	@Autowired
	protected IDaSalePeasantUntDal daSalePeasantUntDal;


	public ResultModel updateList(DaSalePeasantParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaSalePeasantParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaSalePeasantParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaSalePeasantParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaSalePeasantParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaSalePeasantParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaSalePeasantParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaSalePeasant> list = baseUntDal.getList(map);
			PageInfo<DaSalePeasant> pageInfo = new PageInfo<DaSalePeasant>(list);
			List<DaSalePeasant> modelList = pageInfo.getList();
