...
import com.zee.dao.unity.da.IDaInputUntDal;
 ...
 * @description 投入品信息表 业务逻辑处理类，扩展自BaseUntBll<DaInput>，自动生成。
 */
public class DaInputGenUntBll extends BaseUntBll<DaInput> {

	@Autowired
	protected IDaInputUntDal daInputUntDal;


	public ResultModel updateList(DaInputParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaInputParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaInputParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaInputParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaInputParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaInputParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaInputParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaInput> list = baseUntDal.getList(map);
			PageInfo<DaInput> pageInfo = new PageInfo<DaInput>(list);
			List<DaInput> modelList = pageInfo.getList();
