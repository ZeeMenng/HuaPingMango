...
import com.jusfoun.dao.unity.pi.IPirContentImageUntDal;
 ...
 * @description CMS文章内容所包含图片信息表 业务逻辑处理类，扩展自BaseUntBll<PirContentImage>，自动生成。
 */
public class PirContentImageGenUntBll extends BaseUntBll<PirContentImage> {

	@Autowired
	protected IPirContentImageUntDal pirContentImageUntDal;


	public ResultModel updateList(PirContentImageParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirContentImageParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirContentImageParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirContentImageParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirContentImageParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirContentImageParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirContentImageParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirContentImage> list = baseUntDal.getList(map);
			PageInfo<PirContentImage> pageInfo = new PageInfo<PirContentImage>(list);
			List<PirContentImage> modelList = pageInfo.getList();
