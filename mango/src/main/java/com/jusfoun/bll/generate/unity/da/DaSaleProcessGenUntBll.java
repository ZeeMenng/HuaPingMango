...
import com.jusfoun.dao.unity.da.IDaSaleProcessUntDal;
 ...
 * @description 加工销售数据表 业务逻辑处理类，扩展自BaseUntBll<DaSaleProcess>，自动生成。
 */
public class DaSaleProcessGenUntBll extends BaseUntBll<DaSaleProcess> {

	@Autowired
	protected IDaSaleProcessUntDal daSaleProcessUntDal;


	public ResultModel updateList(DaSaleProcessParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaSaleProcessParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaSaleProcessParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaSaleProcessParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaSaleProcessParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaSaleProcessParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaSaleProcessParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaSaleProcess> list = baseUntDal.getList(map);
			PageInfo<DaSaleProcess> pageInfo = new PageInfo<DaSaleProcess>(list);
			List<DaSaleProcess> modelList = pageInfo.getList();
