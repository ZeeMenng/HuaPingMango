...
import com.jusfoun.dao.unity.da.IDaImportExportUntDal;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.extend.da.DaImportExport;
import com.jusfoun.ent.parameter.da.DaImportExportParameter;
 ...
 * @description 进出口数据表，用于存放直报数据的进出口数据 业务逻辑处理类，扩展自BaseUntBll<DaImportExport>，自动生成。
 */
public class DaImportExportGenUntBll extends BaseUntBll<DaImportExport> {

	@Autowired
	protected IDaImportExportUntDal daImportExportUntDal;


	public ResultModel updateList(DaImportExportParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaImportExportParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaImportExportParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaImportExportParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaImportExportParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaImportExportParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaImportExportParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaImportExport> list = baseUntDal.getList(map);
			PageInfo<DaImportExport> pageInfo = new PageInfo<DaImportExport>(list);
			List<DaImportExport> modelList = pageInfo.getList();
