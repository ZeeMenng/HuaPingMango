...
import com.jusfoun.dao.unity.pi.IPiTopicUntDal;
 ...
 * @description CMS专题表 业务逻辑处理类，扩展自BaseUntBll<PiTopic>，自动生成。
 */
public class PiTopicGenUntBll extends BaseUntBll<PiTopic> {

	@Autowired
	protected IPiTopicUntDal piTopicUntDal;


	public ResultModel updateList(PiTopicParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiTopicParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiTopicParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiTopicParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiTopicParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiTopicParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiTopicParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiTopic> list = baseUntDal.getList(map);
			PageInfo<PiTopic> pageInfo = new PageInfo<PiTopic>(list);
			List<PiTopic> modelList = pageInfo.getList();
