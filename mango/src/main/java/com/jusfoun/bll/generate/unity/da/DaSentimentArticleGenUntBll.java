...
import com.jusfoun.dao.unity.da.IDaSentimentArticleUntDal;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.extend.da.DaSentimentArticle;
import com.jusfoun.ent.parameter.da.DaSentimentArticleParameter;
 ...
 * @description 舆情文章表 业务逻辑处理类，扩展自BaseUntBll<DaSentimentArticle>，自动生成。
 */
public class DaSentimentArticleGenUntBll extends BaseUntBll<DaSentimentArticle> {

	@Autowired
	protected IDaSentimentArticleUntDal daSentimentArticleUntDal;


	public ResultModel updateList(DaSentimentArticleParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaSentimentArticleParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaSentimentArticleParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaSentimentArticleParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaSentimentArticleParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaSentimentArticleParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaSentimentArticleParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaSentimentArticle> list = baseUntDal.getList(map);
			PageInfo<DaSentimentArticle> pageInfo = new PageInfo<DaSentimentArticle>(list);
			List<DaSentimentArticle> modelList = pageInfo.getList();
