...
import com.jusfoun.dao.unity.da.IDaBaseDetailUntDal;
 ...
 * @description 基地细分表-地块表 业务逻辑处理类，扩展自BaseUntBll<DaBaseDetail>，自动生成。
 */
public class DaBaseDetailGenUntBll extends BaseUntBll<DaBaseDetail> {

	@Autowired
	protected IDaBaseDetailUntDal daBaseDetailUntDal;


	public ResultModel updateList(DaBaseDetailParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaBaseDetailParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaBaseDetailParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaBaseDetailParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaBaseDetailParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaBaseDetailParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaBaseDetailParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaBaseDetail> list = baseUntDal.getList(map);
			PageInfo<DaBaseDetail> pageInfo = new PageInfo<DaBaseDetail>(list);
			List<DaBaseDetail> modelList = pageInfo.getList();
