...
import com.jusfoun.dao.unity.pi.IPiContentCountUntDal;
 ...
 * @description CMS内容计数表 业务逻辑处理类，扩展自BaseUntBll<PiContentCount>，自动生成。
 */
public class PiContentCountGenUntBll extends BaseUntBll<PiContentCount> {

	@Autowired
	protected IPiContentCountUntDal piContentCountUntDal;


	public ResultModel updateList(PiContentCountParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentCountParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentCountParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentCountParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentCountParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentCountParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentCountParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentCount> list = baseUntDal.getList(map);
			PageInfo<PiContentCount> pageInfo = new PageInfo<PiContentCount>(list);
			List<PiContentCount> modelList = pageInfo.getList();
