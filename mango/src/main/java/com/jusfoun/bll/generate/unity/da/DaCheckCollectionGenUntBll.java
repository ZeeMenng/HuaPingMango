...
import com.jusfoun.dao.unity.da.IDaCheckCollectionUntDal;
 ...
 * @updateDate 2018/7/27 9:57:01
 * @description 采样数据表 业务逻辑处理类，扩展自BaseUntBll<DaCheckCollection>，自动生成。
 */
public class DaCheckCollectionGenUntBll extends BaseUntBll<DaCheckCollection> {

	@Autowired
	protected IDaCheckCollectionUntDal daCheckCollectionUntDal;


	public ResultModel updateList(DaCheckCollectionParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaCheckCollectionParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaCheckCollectionParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaCheckCollectionParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaCheckCollectionParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaCheckCollectionParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaCheckCollectionParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaCheckCollection> list = baseUntDal.getList(map);
			PageInfo<DaCheckCollection> pageInfo = new PageInfo<DaCheckCollection>(list);
			List<DaCheckCollection> modelList = pageInfo.getList();
