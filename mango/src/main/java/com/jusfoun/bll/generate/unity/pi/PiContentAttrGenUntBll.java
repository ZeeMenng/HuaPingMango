...
import com.jusfoun.dao.unity.pi.IPiContentAttrUntDal;
 ...
 * @description CMS内容扩展属性表 业务逻辑处理类，扩展自BaseUntBll<PiContentAttr>，自动生成。
 */
public class PiContentAttrGenUntBll extends BaseUntBll<PiContentAttr> {

	@Autowired
	protected IPiContentAttrUntDal piContentAttrUntDal;


	public ResultModel updateList(PiContentAttrParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentAttrParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentAttrParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentAttrParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentAttrParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentAttrParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentAttrParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentAttr> list = baseUntDal.getList(map);
			PageInfo<PiContentAttr> pageInfo = new PageInfo<PiContentAttr>(list);
			List<PiContentAttr> modelList = pageInfo.getList();
