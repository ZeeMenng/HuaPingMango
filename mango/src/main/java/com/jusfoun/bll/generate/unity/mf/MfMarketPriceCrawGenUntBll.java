...
import com.jusfoun.dao.unity.mf.IMfMarketPriceCrawUntDal;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.extend.mf.MfMarketPriceCraw;
import com.jusfoun.ent.parameter.mf.MfMarketPriceCrawParameter;
 ...
 * @description 市场价格数据表（采集） 业务逻辑处理类，扩展自BaseUntBll<MfMarketPriceCraw>，自动生成。
 */
public class MfMarketPriceCrawGenUntBll extends BaseUntBll<MfMarketPriceCraw> {

	@Autowired
	protected IMfMarketPriceCrawUntDal mfMarketPriceCrawUntDal;


	public ResultModel updateList(MfMarketPriceCrawParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfMarketPriceCrawParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfMarketPriceCrawParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfMarketPriceCrawParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfMarketPriceCrawParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfMarketPriceCrawParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfMarketPriceCrawParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfMarketPriceCraw> list = baseUntDal.getList(map);
			PageInfo<MfMarketPriceCraw> pageInfo = new PageInfo<MfMarketPriceCraw>(list);
			List<MfMarketPriceCraw> modelList = pageInfo.getList();
