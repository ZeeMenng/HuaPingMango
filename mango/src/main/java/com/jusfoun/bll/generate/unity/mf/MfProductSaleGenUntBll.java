...
import com.jusfoun.dao.unity.mf.IMfProductSaleUntDal;
 ...
 * @description 产销预测表 业务逻辑处理类，扩展自BaseUntBll<MfProductSale>，自动生成。
 */
public class MfProductSaleGenUntBll extends BaseUntBll<MfProductSale> {

	@Autowired
	protected IMfProductSaleUntDal mfProductSaleUntDal;


	public ResultModel updateList(MfProductSaleParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfProductSaleParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfProductSaleParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfProductSaleParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfProductSaleParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfProductSaleParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfProductSaleParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfProductSale> list = baseUntDal.getList(map);
			PageInfo<MfProductSale> pageInfo = new PageInfo<MfProductSale>(list);
			List<MfProductSale> modelList = pageInfo.getList();
