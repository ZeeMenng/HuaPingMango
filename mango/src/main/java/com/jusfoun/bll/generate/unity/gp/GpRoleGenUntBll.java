...
import com.jusfoun.dao.unity.gp.IGpRoleUntDal;
...
public class GpRoleGenUntBll extends BaseUntBll<GpRole> {
	...
	@Autowired
	protected IGpRoleUntDal gpRoleUntDal;


	public ResultModel updateList(GpRoleParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpRoleParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpRoleParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpRoleParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpRoleParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpRoleParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpRoleParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpRole> list = baseUntDal.getList(map);
			PageInfo<GpRole> pageInfo = new PageInfo<GpRole>(list);
			List<GpRole> modelList = pageInfo.getList();
