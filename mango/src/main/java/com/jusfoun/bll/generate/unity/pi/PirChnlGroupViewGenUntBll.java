...
import com.jusfoun.dao.unity.pi.IPirChnlGroupViewUntDal;
 ...
 * @description CMS栏目浏览会员组关联表 业务逻辑处理类，扩展自BaseUntBll<PirChnlGroupView>，自动生成。
 */
public class PirChnlGroupViewGenUntBll extends BaseUntBll<PirChnlGroupView> {

	@Autowired
	protected IPirChnlGroupViewUntDal pirChnlGroupViewUntDal;


	public ResultModel updateList(PirChnlGroupViewParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirChnlGroupViewParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirChnlGroupViewParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirChnlGroupViewParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirChnlGroupViewParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirChnlGroupViewParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirChnlGroupViewParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirChnlGroupView> list = baseUntDal.getList(map);
			PageInfo<PirChnlGroupView> pageInfo = new PageInfo<PirChnlGroupView>(list);
			List<PirChnlGroupView> modelList = pageInfo.getList();
