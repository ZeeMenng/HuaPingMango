...
import com.zee.dao.unity.mf.IMfHpSeasonUntDal;
 ...
 * @description 批发价格季节性波动规律分析 业务逻辑处理类，扩展自BaseUntBll<MfHpSeason>，自动生成。
 */
public class MfHpSeasonGenUntBll extends BaseUntBll<MfHpSeason> {

	@Autowired
	protected IMfHpSeasonUntDal mfHpSeasonUntDal;


	public ResultModel updateList(MfHpSeasonParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfHpSeasonParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfHpSeasonParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfHpSeasonParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfHpSeasonParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfHpSeasonParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfHpSeasonParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfHpSeason> list = baseUntDal.getList(map);
			PageInfo<MfHpSeason> pageInfo = new PageInfo<MfHpSeason>(list);
			List<MfHpSeason> modelList = pageInfo.getList();
