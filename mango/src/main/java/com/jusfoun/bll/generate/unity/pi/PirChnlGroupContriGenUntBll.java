...
import com.jusfoun.dao.unity.pi.IPirChnlGroupContriUntDal;
 ...
 * @description CMS栏目投稿会员组关联表 业务逻辑处理类，扩展自BaseUntBll<PirChnlGroupContri>，自动生成。
 */
public class PirChnlGroupContriGenUntBll extends BaseUntBll<PirChnlGroupContri> {

	@Autowired
	protected IPirChnlGroupContriUntDal pirChnlGroupContriUntDal;


	public ResultModel updateList(PirChnlGroupContriParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirChnlGroupContriParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirChnlGroupContriParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirChnlGroupContriParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirChnlGroupContriParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirChnlGroupContriParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirChnlGroupContriParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirChnlGroupContri> list = baseUntDal.getList(map);
			PageInfo<PirChnlGroupContri> pageInfo = new PageInfo<PirChnlGroupContri>(list);
			List<PirChnlGroupContri> modelList = pageInfo.getList();
