...
import com.jusfoun.dao.unity.pi.IPiContentUntDal;
 ...
 * @description CMS内容表 业务逻辑处理类，扩展自BaseUntBll<PiContent>，自动生成。
 */
public class PiContentGenUntBll extends BaseUntBll<PiContent> {

	@Autowired
	protected IPiContentUntDal piContentUntDal;


	public ResultModel updateList(PiContentParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContent> list = baseUntDal.getList(map);
			PageInfo<PiContent> pageInfo = new PageInfo<PiContent>(list);
			List<PiContent> modelList = pageInfo.getList();
