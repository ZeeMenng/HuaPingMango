...
import com.jusfoun.dao.unity.mf.IMfQualityInspectionUntDal;
 ...
 * @description 质量抽检表 业务逻辑处理类，扩展自BaseUntBll<MfQualityInspection>，自动生成。
 */
public class MfQualityInspectionGenUntBll extends BaseUntBll<MfQualityInspection> {

	@Autowired
	protected IMfQualityInspectionUntDal mfQualityInspectionUntDal;


	public ResultModel updateList(MfQualityInspectionParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfQualityInspectionParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfQualityInspectionParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfQualityInspectionParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfQualityInspectionParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfQualityInspectionParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfQualityInspectionParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfQualityInspection> list = baseUntDal.getList(map);
			PageInfo<MfQualityInspection> pageInfo = new PageInfo<MfQualityInspection>(list);
			List<MfQualityInspection> modelList = pageInfo.getList();
