...
import com.jusfoun.dao.unity.pe.IPeAerialBroadcastUntDal;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.extend.pe.PeAerialBroadcast;
import com.jusfoun.ent.parameter.pe.PeAerialBroadcastParameter;
 ...
 * @description 鸟瞰华坪轮播图 业务逻辑处理类，扩展自BaseUntBll<PeAerialBroadcast>，自动生成。
 */
public class PeAerialBroadcastGenUntBll extends BaseUntBll<PeAerialBroadcast> {

	@Autowired
	protected IPeAerialBroadcastUntDal peAerialBroadcastUntDal;


	public ResultModel updateList(PeAerialBroadcastParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PeAerialBroadcastParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PeAerialBroadcastParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PeAerialBroadcastParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PeAerialBroadcastParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PeAerialBroadcastParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PeAerialBroadcastParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PeAerialBroadcast> list = baseUntDal.getList(map);
			PageInfo<PeAerialBroadcast> pageInfo = new PageInfo<PeAerialBroadcast>(list);
			List<PeAerialBroadcast> modelList = pageInfo.getList();
