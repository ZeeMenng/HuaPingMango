...
import com.zee.dao.unity.mf.IMfFormatrixUntDal;
 ...
 * @description 波士顿矩阵 业务逻辑处理类，扩展自BaseUntBll<MfFormatrix>，自动生成。
 */
public class MfFormatrixGenUntBll extends BaseUntBll<MfFormatrix> {

	@Autowired
	protected IMfFormatrixUntDal mfFormatrixUntDal;


	public ResultModel updateList(MfFormatrixParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfFormatrixParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfFormatrixParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfFormatrixParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfFormatrixParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfFormatrixParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfFormatrixParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfFormatrix> list = baseUntDal.getList(map);
			PageInfo<MfFormatrix> pageInfo = new PageInfo<MfFormatrix>(list);
			List<MfFormatrix> modelList = pageInfo.getList();
