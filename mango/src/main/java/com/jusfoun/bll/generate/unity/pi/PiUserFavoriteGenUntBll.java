...
import com.jusfoun.dao.unity.pi.IPiUserFavoriteUntDal;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.extend.pi.PiUserFavorite;
import com.jusfoun.ent.parameter.pi.PiUserFavoriteParameter;
...
public class PiUserFavoriteGenUntBll extends BaseUntBll<PiUserFavorite> {
	...
	@Autowired
	protected IPiUserFavoriteUntDal piUserFavoriteUntDal;


	public ResultModel updateList(PiUserFavoriteParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiUserFavoriteParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiUserFavoriteParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiUserFavoriteParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiUserFavoriteParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiUserFavoriteParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiUserFavoriteParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiUserFavorite> list = baseUntDal.getList(map);
			PageInfo<PiUserFavorite> pageInfo = new PageInfo<PiUserFavorite>(list);
			List<PiUserFavorite> modelList = pageInfo.getList();
