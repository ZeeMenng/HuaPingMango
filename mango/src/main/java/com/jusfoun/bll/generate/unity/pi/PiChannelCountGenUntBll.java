...
import com.jusfoun.dao.unity.pi.IPiChannelCountUntDal;
 ...
 * @description CMS栏目访问量计数表 业务逻辑处理类，扩展自BaseUntBll<PiChannelCount>，自动生成。
 */
public class PiChannelCountGenUntBll extends BaseUntBll<PiChannelCount> {

	@Autowired
	protected IPiChannelCountUntDal piChannelCountUntDal;


	public ResultModel updateList(PiChannelCountParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiChannelCountParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiChannelCountParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiChannelCountParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiChannelCountParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiChannelCountParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiChannelCountParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiChannelCount> list = baseUntDal.getList(map);
			PageInfo<PiChannelCount> pageInfo = new PageInfo<PiChannelCount>(list);
			List<PiChannelCount> modelList = pageInfo.getList();
