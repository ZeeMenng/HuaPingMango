...
import com.jusfoun.dao.unity.pi.IPiContentAttachmentUntDal;
 ...
 * @description CMS内容附件表 业务逻辑处理类，扩展自BaseUntBll<PiContentAttachment>，自动生成。
 */
public class PiContentAttachmentGenUntBll extends BaseUntBll<PiContentAttachment> {

	@Autowired
	protected IPiContentAttachmentUntDal piContentAttachmentUntDal;


	public ResultModel updateList(PiContentAttachmentParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentAttachmentParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentAttachmentParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentAttachmentParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentAttachmentParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentAttachmentParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentAttachmentParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentAttachment> list = baseUntDal.getList(map);
			PageInfo<PiContentAttachment> pageInfo = new PageInfo<PiContentAttachment>(list);
			List<PiContentAttachment> modelList = pageInfo.getList();
