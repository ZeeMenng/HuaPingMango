...
import com.jusfoun.dao.unity.da.IDaSaleFreshUntDal;
 ...
 * @description 鲜果销售数据表 业务逻辑处理类，扩展自BaseUntBll<DaSaleFresh>，自动生成。
 */
public class DaSaleFreshGenUntBll extends BaseUntBll<DaSaleFresh> {

	@Autowired
	protected IDaSaleFreshUntDal daSaleFreshUntDal;


	public ResultModel updateList(DaSaleFreshParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaSaleFreshParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaSaleFreshParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaSaleFreshParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaSaleFreshParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaSaleFreshParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaSaleFreshParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaSaleFresh> list = baseUntDal.getList(map);
			PageInfo<DaSaleFresh> pageInfo = new PageInfo<DaSaleFresh>(list);
			List<DaSaleFresh> modelList = pageInfo.getList();
