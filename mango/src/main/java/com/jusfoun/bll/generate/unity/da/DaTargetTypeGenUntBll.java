...
import com.jusfoun.dao.unity.da.IDaTargetTypeUntDal;
 ...
 * @description 指标类型 业务逻辑处理类，扩展自BaseUntBll<DaTargetType>，自动生成。
 */
public class DaTargetTypeGenUntBll extends BaseUntBll<DaTargetType> {

	@Autowired
	protected IDaTargetTypeUntDal daTargetTypeUntDal;


	public ResultModel updateList(DaTargetTypeParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaTargetTypeParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaTargetTypeParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaTargetTypeParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaTargetTypeParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaTargetTypeParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaTargetTypeParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaTargetType> list = baseUntDal.getList(map);
			PageInfo<DaTargetType> pageInfo = new PageInfo<DaTargetType>(list);
			List<DaTargetType> modelList = pageInfo.getList();
