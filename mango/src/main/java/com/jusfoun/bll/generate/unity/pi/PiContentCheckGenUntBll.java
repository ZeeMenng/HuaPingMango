...
import com.jusfoun.dao.unity.pi.IPiContentCheckUntDal;
 ...
 * @description CMS内容审核信息表 业务逻辑处理类，扩展自BaseUntBll<PiContentCheck>，自动生成。
 */
public class PiContentCheckGenUntBll extends BaseUntBll<PiContentCheck> {

	@Autowired
	protected IPiContentCheckUntDal piContentCheckUntDal;


	public ResultModel updateList(PiContentCheckParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentCheckParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentCheckParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentCheckParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentCheckParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentCheckParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentCheckParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentCheck> list = baseUntDal.getList(map);
			PageInfo<PiContentCheck> pageInfo = new PageInfo<PiContentCheck>(list);
			List<PiContentCheck> modelList = pageInfo.getList();
