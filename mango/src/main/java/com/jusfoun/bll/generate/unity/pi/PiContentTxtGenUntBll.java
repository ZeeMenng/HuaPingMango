...
import com.jusfoun.dao.unity.pi.IPiContentTxtUntDal;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.extend.pi.PiContentTxt;
import com.jusfoun.ent.parameter.pi.PiContentTxtParameter;
 ...
 * @description CMS内容文本表 业务逻辑处理类，扩展自BaseUntBll<PiContentTxt>，自动生成。
 */
public class PiContentTxtGenUntBll extends BaseUntBll<PiContentTxt> {

	@Autowired
	protected IPiContentTxtUntDal piContentTxtUntDal;


	public ResultModel updateList(PiContentTxtParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentTxtParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentTxtParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentTxtParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentTxtParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentTxtParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentTxtParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentTxt> list = baseUntDal.getList(map);
			PageInfo<PiContentTxt> pageInfo = new PageInfo<PiContentTxt>(list);
			List<PiContentTxt> modelList = pageInfo.getList();
