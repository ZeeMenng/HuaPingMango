...
import com.jusfoun.dao.unity.pi.IPiAcquisitionUntDal;
 ...
 * @description CMS采集任务表 业务逻辑处理类，扩展自BaseUntBll<PiAcquisition>，自动生成。
 */
public class PiAcquisitionGenUntBll extends BaseUntBll<PiAcquisition> {

	@Autowired
	protected IPiAcquisitionUntDal piAcquisitionUntDal;


	public ResultModel updateList(PiAcquisitionParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiAcquisitionParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiAcquisitionParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiAcquisitionParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiAcquisitionParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiAcquisitionParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiAcquisitionParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiAcquisition> list = baseUntDal.getList(map);
			PageInfo<PiAcquisition> pageInfo = new PageInfo<PiAcquisition>(list);
			List<PiAcquisition> modelList = pageInfo.getList();
