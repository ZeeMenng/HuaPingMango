...
import com.jusfoun.dao.unity.da.IDaCrawlerUntDal;
 ...
 * @description 爬虫 业务逻辑处理类，扩展自BaseUntBll<DaCrawler>，自动生成。
 */
public class DaCrawlerGenUntBll extends BaseUntBll<DaCrawler> {

	@Autowired
	protected IDaCrawlerUntDal daCrawlerUntDal;


	public ResultModel updateList(DaCrawlerParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaCrawlerParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaCrawlerParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaCrawlerParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaCrawlerParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaCrawlerParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaCrawlerParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaCrawler> list = baseUntDal.getList(map);
			PageInfo<DaCrawler> pageInfo = new PageInfo<DaCrawler>(list);
			List<DaCrawler> modelList = pageInfo.getList();
