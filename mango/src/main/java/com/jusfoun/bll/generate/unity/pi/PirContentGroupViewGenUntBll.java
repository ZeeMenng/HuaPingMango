...
import com.jusfoun.dao.unity.pi.IPirContentGroupViewUntDal;
 ...
 * @description CMS内容浏览会员组关联表 业务逻辑处理类，扩展自BaseUntBll<PirContentGroupView>，自动生成。
 */
public class PirContentGroupViewGenUntBll extends BaseUntBll<PirContentGroupView> {

	@Autowired
	protected IPirContentGroupViewUntDal pirContentGroupViewUntDal;


	public ResultModel updateList(PirContentGroupViewParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirContentGroupViewParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirContentGroupViewParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirContentGroupViewParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirContentGroupViewParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirContentGroupViewParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirContentGroupViewParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirContentGroupView> list = baseUntDal.getList(map);
			PageInfo<PirContentGroupView> pageInfo = new PageInfo<PirContentGroupView>(list);
			List<PirContentGroupView> modelList = pageInfo.getList();
