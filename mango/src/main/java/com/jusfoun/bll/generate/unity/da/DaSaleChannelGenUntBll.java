...
import com.jusfoun.dao.unity.da.IDaSaleChannelUntDal;
 ...
 * @description 销售渠道数据表 业务逻辑处理类，扩展自BaseUntBll<DaSaleChannel>，自动生成。
 */
public class DaSaleChannelGenUntBll extends BaseUntBll<DaSaleChannel> {

	@Autowired
	protected IDaSaleChannelUntDal daSaleChannelUntDal;


	public ResultModel updateList(DaSaleChannelParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaSaleChannelParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaSaleChannelParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaSaleChannelParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaSaleChannelParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaSaleChannelParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaSaleChannelParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaSaleChannel> list = baseUntDal.getList(map);
			PageInfo<DaSaleChannel> pageInfo = new PageInfo<DaSaleChannel>(list);
			List<DaSaleChannel> modelList = pageInfo.getList();
