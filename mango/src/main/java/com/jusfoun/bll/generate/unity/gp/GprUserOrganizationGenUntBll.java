...
import com.jusfoun.dao.unity.gp.IGprUserOrganizationUntDal;
 ...
 * @description 用户所属组织机构。 业务逻辑处理类，扩展自BaseUntBll<GprUserOrganization>，自动生成。
 */
public class GprUserOrganizationGenUntBll extends BaseUntBll<GprUserOrganization> {

	@Autowired
	protected IGprUserOrganizationUntDal gprUserOrganizationUntDal;


	public ResultModel updateList(GprUserOrganizationParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GprUserOrganizationParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GprUserOrganizationParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GprUserOrganizationParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GprUserOrganizationParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GprUserOrganizationParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GprUserOrganizationParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GprUserOrganization> list = baseUntDal.getList(map);
			PageInfo<GprUserOrganization> pageInfo = new PageInfo<GprUserOrganization>(list);
			List<GprUserOrganization> modelList = pageInfo.getList();
 	...
 	public ResultModel deleteByOrganizationId(String organizationId) {
		return deleteByOrganizationId(organizationId, isLogRead);
	}

	public ResultModel deleteByOrganizationId(String organizationId, boolean isLog) {
		ResultModel result = new ResultModel();
		try {
			result.setAddTime(DateUtils.getCurrentTime());
			result.setId(Tools.getUUID());
			result.setIncomeValue(organizationId);
			result.setTableName(this.getClass().getSimpleName());
			...
			int i = gprUserOrganizationUntDal.deleteByOrganizationId( organizationId);
	...
	public ResultModel deleteByOrganizationIdList(ArrayList<String> organizationIdList, boolean isLog) {
		...
		try {
			...
			int i = gprUserOrganizationUntDal.deleteByOrganizationIdList(organizationIdList);
	...
	public ResultModel getListByOrganizationId(String organizationId, boolean isLog) {
		ResultModel result = new ResultModel();
		try {
			result.setAddTime(DateUtils.getCurrentTime());
			result.setId(Tools.getUUID());
			...
			List<GprUserOrganization> modelList = gprUserOrganizationUntDal.getListByOrganizationId(organizationId);
	...
	public ResultModel deleteByUserId(String userId, boolean isLog) {
		...
		try {
			...
			int i = gprUserOrganizationUntDal.deleteByUserId( userId);
	...
	public ResultModel deleteByUserIdList(ArrayList<String> userIdList, boolean isLog) {
		...
		try {
			...
			int i = gprUserOrganizationUntDal.deleteByUserIdList(userIdList);
	...
	public ResultModel getListByUserId(String userId, boolean isLog) {
		...
		try {
			...
			List<GprUserOrganization> modelList = gprUserOrganizationUntDal.getListByUserId(userId);
 	...
 	public ResultModel deleteByCompositeIdList(ArrayList<GprUserOrganization> gprUserOrganizationList) {
		return deleteByCompositeIdList(gprUserOrganizationList, isLogWrite);
	}

	public ResultModel deleteByCompositeIdList(ArrayList<GprUserOrganization> gprUserOrganizationList, boolean isLog) {
		ResultModel result = new ResultModel();
		if (gprUserOrganizationList == null || gprUserOrganizationList.isEmpty()) {
		...
		try {
			result.setId(Tools.getUUID());
			result.setIncomeValue(JSONArray.fromObject(gprUserOrganizationList).toString());
			result.setAddTime(DateUtils.getCurrentTime());
			result.setTableName(this.getClass().getSimpleName());
			result.setOperTypeCode(OperType.DELETELIST.getCode());
			result.setOperTypeText(OperType.DELETELIST.getText());
			...
			int i = gprUserOrganizationUntDal.deleteByCompositeIdList(gprUserOrganizationList);
			...
			result.setResultMessage(OperResult.DELETELIST_S.getText());
			result.setIsSuccessCode(SymbolicConstant.DCODE_BOOLEAN_T);
			if (i != gprUserOrganizationList.size()) {
