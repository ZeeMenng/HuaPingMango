...
import com.jusfoun.dao.unity.pi.IPiContentPictureUntDal;
 ...
 * @updateDate 2018/5/24 0:41:04
 * @description CMS内容图片表 业务逻辑处理类，扩展自BaseUntBll<PiContentPicture>，自动生成。
 */
public class PiContentPictureGenUntBll extends BaseUntBll<PiContentPicture> {

	@Autowired
	protected IPiContentPictureUntDal piContentPictureUntDal;


	public ResultModel updateList(PiContentPictureParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentPictureParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentPictureParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentPictureParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentPictureParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentPictureParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentPictureParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentPicture> list = baseUntDal.getList(map);
			PageInfo<PiContentPicture> pageInfo = new PageInfo<PiContentPicture>(list);
			List<PiContentPicture> modelList = pageInfo.getList();
