...
import com.jusfoun.dao.unity.da.IDaUserContributionUntDal;
 ...
 * @updateDate 2018-6-19 16:50:21
 * @description 用户贡献 业务逻辑处理类，扩展自BaseUntBll<DaUserContribution>，自动生成。
 */
public class DaUserContributionGenUntBll extends BaseUntBll<DaUserContribution> {

	@Autowired
	protected IDaUserContributionUntDal daUserContributionUntDal;


	public ResultModel updateList(DaUserContributionParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaUserContributionParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaUserContributionParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaUserContributionParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaUserContributionParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaUserContributionParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaUserContributionParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaUserContribution> list = baseUntDal.getList(map);
			PageInfo<DaUserContribution> pageInfo = new PageInfo<DaUserContribution>(list);
			List<DaUserContribution> modelList = pageInfo.getList();
