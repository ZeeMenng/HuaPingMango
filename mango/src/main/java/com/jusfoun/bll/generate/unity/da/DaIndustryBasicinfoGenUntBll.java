...
import com.zee.dao.unity.da.IDaIndustryBasicinfoUntDal;
 ...
 * @updateDate 2021/1/28 16:06:54
 * @description 产业基本情况表 业务逻辑处理类，扩展自BaseUntBll<DaIndustryBasicinfo>，自动生成。
 */
public class DaIndustryBasicinfoGenUntBll extends BaseUntBll<DaIndustryBasicinfo> {

	@Autowired
	protected IDaIndustryBasicinfoUntDal daIndustryBasicinfoUntDal;


	public ResultModel updateList(DaIndustryBasicinfoParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaIndustryBasicinfoParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaIndustryBasicinfoParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaIndustryBasicinfoParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaIndustryBasicinfoParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaIndustryBasicinfoParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaIndustryBasicinfoParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaIndustryBasicinfo> list = baseUntDal.getList(map);
			PageInfo<DaIndustryBasicinfo> pageInfo = new PageInfo<DaIndustryBasicinfo>(list);
			List<DaIndustryBasicinfo> modelList = pageInfo.getList();
