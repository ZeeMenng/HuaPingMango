...
import com.jusfoun.dao.unity.pi.IPiAcquisitionHistoryUntDal;
 ...
 * @updateDate 2018/5/24 0:41:03
 * @description CMS采集的文章历史记录表 业务逻辑处理类，扩展自BaseUntBll<PiAcquisitionHistory>，自动生成。
 */
public class PiAcquisitionHistoryGenUntBll extends BaseUntBll<PiAcquisitionHistory> {

	@Autowired
	protected IPiAcquisitionHistoryUntDal piAcquisitionHistoryUntDal;


	public ResultModel updateList(PiAcquisitionHistoryParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiAcquisitionHistoryParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiAcquisitionHistoryParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiAcquisitionHistoryParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiAcquisitionHistoryParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiAcquisitionHistoryParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiAcquisitionHistoryParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiAcquisitionHistory> list = baseUntDal.getList(map);
			PageInfo<PiAcquisitionHistory> pageInfo = new PageInfo<PiAcquisitionHistory>(list);
			List<PiAcquisitionHistory> modelList = pageInfo.getList();
