...
import com.jusfoun.dao.unity.mf.IMfFreshVolumeUntDal;
 ...
 * @description mf_fresh_volume 业务逻辑处理类，扩展自BaseUntBll<MfFreshVolume>，自动生成。
 */
public class MfFreshVolumeGenUntBll extends BaseUntBll<MfFreshVolume> {

	@Autowired
	protected IMfFreshVolumeUntDal mfFreshVolumeUntDal;


	public ResultModel updateList(MfFreshVolumeParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfFreshVolumeParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfFreshVolumeParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfFreshVolumeParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfFreshVolumeParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfFreshVolumeParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfFreshVolumeParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfFreshVolume> list = baseUntDal.getList(map);
			PageInfo<MfFreshVolume> pageInfo = new PageInfo<MfFreshVolume>(list);
			List<MfFreshVolume> modelList = pageInfo.getList();
