...
import com.jusfoun.dao.unity.pi.IPiContentTagUntDal;
 ...
 * @description CMS内容TAG表 业务逻辑处理类，扩展自BaseUntBll<PiContentTag>，自动生成。
 */
public class PiContentTagGenUntBll extends BaseUntBll<PiContentTag> {

	@Autowired
	protected IPiContentTagUntDal piContentTagUntDal;


	public ResultModel updateList(PiContentTagParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiContentTagParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiContentTagParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiContentTagParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiContentTagParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiContentTagParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiContentTagParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiContentTag> list = baseUntDal.getList(map);
			PageInfo<PiContentTag> pageInfo = new PageInfo<PiContentTag>(list);
			List<PiContentTag> modelList = pageInfo.getList();
