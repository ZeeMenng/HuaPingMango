...
import com.jusfoun.dao.unity.pi.IPiChannelExtUntDal;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.extend.pi.PiChannelExt;
import com.jusfoun.ent.parameter.pi.PiChannelExtParameter;
 ...
 * @description CMS栏目内容表 业务逻辑处理类，扩展自BaseUntBll<PiChannelExt>，自动生成。
 */
public class PiChannelExtGenUntBll extends BaseUntBll<PiChannelExt> {

	@Autowired
	protected IPiChannelExtUntDal piChannelExtUntDal;


	public ResultModel updateList(PiChannelExtParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiChannelExtParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiChannelExtParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiChannelExtParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiChannelExtParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiChannelExtParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiChannelExtParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiChannelExt> list = baseUntDal.getList(map);
			PageInfo<PiChannelExt> pageInfo = new PageInfo<PiChannelExt>(list);
			List<PiChannelExt> modelList = pageInfo.getList();
