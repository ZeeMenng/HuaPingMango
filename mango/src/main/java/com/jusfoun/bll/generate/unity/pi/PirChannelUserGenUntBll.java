...
import com.jusfoun.dao.unity.pi.IPirChannelUserUntDal;
 ...
 * @description CMS栏目用户关联表 业务逻辑处理类，扩展自BaseUntBll<PirChannelUser>，自动生成。
 */
public class PirChannelUserGenUntBll extends BaseUntBll<PirChannelUser> {

	@Autowired
	protected IPirChannelUserUntDal pirChannelUserUntDal;


	public ResultModel updateList(PirChannelUserParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirChannelUserParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirChannelUserParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirChannelUserParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirChannelUserParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirChannelUserParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirChannelUserParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirChannelUser> list = baseUntDal.getList(map);
			PageInfo<PirChannelUser> pageInfo = new PageInfo<PirChannelUser>(list);
			List<PirChannelUser> modelList = pageInfo.getList();
