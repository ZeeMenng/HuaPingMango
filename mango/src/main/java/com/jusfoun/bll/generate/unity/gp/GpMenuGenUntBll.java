...
import com.jusfoun.dao.unity.gp.IGpMenuUntDal;
 ...
 * @description 链接菜单。 业务逻辑处理类，扩展自BaseUntBll<GpMenu>，自动生成。
 */
public class GpMenuGenUntBll extends BaseUntBll<GpMenu> {

	@Autowired
	protected IGpMenuUntDal gpMenuUntDal;


	public ResultModel updateList(GpMenuParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpMenuParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpMenuParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpMenuParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpMenuParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpMenuParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpMenuParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpMenu> list = baseUntDal.getList(map);
			PageInfo<GpMenu> pageInfo = new PageInfo<GpMenu>(list);
			List<GpMenu> modelList = pageInfo.getList();
