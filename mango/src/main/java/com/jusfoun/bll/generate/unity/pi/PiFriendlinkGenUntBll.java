...
import com.jusfoun.dao.unity.pi.IPiFriendlinkUntDal;
 ...
 * @updateDate 2018/7/13 11:38:52
 * @description CMS友情链接 业务逻辑处理类，扩展自BaseUntBll<PiFriendlink>，自动生成。
 */
public class PiFriendlinkGenUntBll extends BaseUntBll<PiFriendlink> {

	@Autowired
	protected IPiFriendlinkUntDal piFriendlinkUntDal;


	public ResultModel updateList(PiFriendlinkParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PiFriendlinkParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PiFriendlinkParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PiFriendlinkParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PiFriendlinkParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PiFriendlinkParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PiFriendlinkParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PiFriendlink> list = baseUntDal.getList(map);
			PageInfo<PiFriendlink> pageInfo = new PageInfo<PiFriendlink>(list);
			List<PiFriendlink> modelList = pageInfo.getList();
