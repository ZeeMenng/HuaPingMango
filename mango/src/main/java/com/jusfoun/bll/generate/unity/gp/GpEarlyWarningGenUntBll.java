...
import com.jusfoun.dao.unity.gp.IGpEarlyWarningUntDal;
 ...
 * @updateDate 2020/10/13 20:02:10
 * @description 预警阀值表 业务逻辑处理类，扩展自BaseUntBll<GpEarlyWarning>，自动生成。
 */
public class GpEarlyWarningGenUntBll extends BaseUntBll<GpEarlyWarning> {

	@Autowired
	protected IGpEarlyWarningUntDal gpEarlyWarningUntDal;


	public ResultModel updateList(GpEarlyWarningParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpEarlyWarningParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpEarlyWarningParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpEarlyWarningParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpEarlyWarningParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpEarlyWarningParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpEarlyWarningParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpEarlyWarning> list = baseUntDal.getList(map);
			PageInfo<GpEarlyWarning> pageInfo = new PageInfo<GpEarlyWarning>(list);
			List<GpEarlyWarning> modelList = pageInfo.getList();
