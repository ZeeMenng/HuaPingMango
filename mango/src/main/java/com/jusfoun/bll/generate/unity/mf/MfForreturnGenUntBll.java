...
import com.jusfoun.dao.unity.mf.IMfForreturnUntDal;
 ...
 * @description 批发价格波动性预测 业务逻辑处理类，扩展自BaseUntBll<MfForreturn>，自动生成。
 */
public class MfForreturnGenUntBll extends BaseUntBll<MfForreturn> {

	@Autowired
	protected IMfForreturnUntDal mfForreturnUntDal;


	public ResultModel updateList(MfForreturnParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(MfForreturnParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(MfForreturnParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(MfForreturnParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			MfForreturnParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			MfForreturnParameter.GetList.Page page = getListParam.getPage();
			ArrayList<MfForreturnParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<MfForreturn> list = baseUntDal.getList(map);
			PageInfo<MfForreturn> pageInfo = new PageInfo<MfForreturn>(list);
			List<MfForreturn> modelList = pageInfo.getList();
