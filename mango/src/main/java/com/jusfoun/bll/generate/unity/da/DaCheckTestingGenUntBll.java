...
import com.jusfoun.dao.unity.da.IDaCheckTestingUntDal;
 ...
 * @updateDate 2018/7/27 10:32:47
 * @description 检测数据表 业务逻辑处理类，扩展自BaseUntBll<DaCheckTesting>，自动生成。
 */
public class DaCheckTestingGenUntBll extends BaseUntBll<DaCheckTesting> {

	@Autowired
	protected IDaCheckTestingUntDal daCheckTestingUntDal;


	public ResultModel updateList(DaCheckTestingParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(DaCheckTestingParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(DaCheckTestingParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(DaCheckTestingParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			DaCheckTestingParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			DaCheckTestingParameter.GetList.Page page = getListParam.getPage();
			ArrayList<DaCheckTestingParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<DaCheckTesting> list = baseUntDal.getList(map);
			PageInfo<DaCheckTesting> pageInfo = new PageInfo<DaCheckTesting>(list);
			List<DaCheckTesting> modelList = pageInfo.getList();
