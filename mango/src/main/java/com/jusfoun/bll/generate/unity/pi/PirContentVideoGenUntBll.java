...
import com.jusfoun.dao.unity.pi.IPirContentVideoUntDal;
 ...
 * @description CMS文章内容所包含视频信息表 业务逻辑处理类，扩展自BaseUntBll<PirContentVideo>，自动生成。
 */
public class PirContentVideoGenUntBll extends BaseUntBll<PirContentVideo> {

	@Autowired
	protected IPirContentVideoUntDal pirContentVideoUntDal;


	public ResultModel updateList(PirContentVideoParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(PirContentVideoParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(PirContentVideoParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(PirContentVideoParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			PirContentVideoParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			PirContentVideoParameter.GetList.Page page = getListParam.getPage();
			ArrayList<PirContentVideoParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<PirContentVideo> list = baseUntDal.getList(map);
			PageInfo<PirContentVideo> pageInfo = new PageInfo<PirContentVideo>(list);
			List<PirContentVideo> modelList = pageInfo.getList();
