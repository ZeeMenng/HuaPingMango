...
import com.jusfoun.bll.extend.split.pi.PiInteractionSplBll;
 ...
 * @description pi_interaction 对外接口，扩展自BaseSwgApp，自动生成。
 */

@Api(value = "PiInteraction",tags="pi_interaction")
@RequestMapping(value = "/generate/swagger/pi/piInteraction")
public class PiInteractionGenSwgApp extends BaseSwgApp {
	...
	@Autowired
	@Qualifier("piInteractionSplBll")
	protected PiInteractionSplBll piInteractionSplBll;

	@ApiOperation(value = "新增记录", notes = "新增单条记录")
	@ApiImplicitParams({ @ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串", required = true, dataType = "PiInteraction") })
	@RequestMapping(value = "/add", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel add(@RequestBody PiInteraction jsonData) {
		ResultModel result = piInteractionUntBll.add(jsonData);

		return result;
	}

	@ApiOperation(value = "批量新增", notes = "新增多条记录")
	@ApiImplicitParams({ @ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串，对象列表", required = true, dataType = "PiInteractionAddList") })
	@RequestMapping(value = "/addList", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel addList(@RequestBody PiInteractionParameter.AddList jsonData) {
		ResultModel result = piInteractionUntBll.add(jsonData.getEntityList());
	...
	public ResultModel delete(@RequestParam String id) {
		ResultModel result = piInteractionUntBll.delete(id);
	...
	@ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串，主键列表", required = true, dataType = "PiInteractionDeleteByIdList")
	@RequestMapping(value = "/deleteList", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel deleteList(@RequestBody PiInteractionParameter.DeleteByIdList jsonData) {
		ResultModel result = piInteractionUntBll.deleteByIdList(jsonData.getIdList());
    ...
    public ResultModel updateListWithDff(@RequestBody PiInteractionParameter.AddList jsonData) {
		ResultModel result = piInteractionUntBll.updateListWithDff(jsonData.getEntityList());
    ...
    public ResultModel updateListWithDffOrAdd(@RequestBody PiInteractionParameter.AddList jsonData) {
		ResultModel result = piInteractionUntBll.updateListWithDffOrAdd(jsonData.getEntityList());
	...
	public ResultModel getListByJsonData() {
		...
		selectBuffer.append("select A.id id,A.interaction_type interactionType,A.nick_name nickName,A.real_name realName,A.job job,A.phone phone,A.email email,A.qq qq,A.region_id regionId,A.region_name regionName,A.title title,A.req_content reqContent,A.res_user_id resUserId,A.res_content resContent,A.audit_status_code auditStatusCode,A.audit_status_text auditStatusText,A.auditer_suggestion auditerSuggestion,A.auditer_user_id auditerUserId,A.auditer_time auditerTime,A.add_time addTime,A.update_time updateTime,A.remark remark  from pi_interaction A inner join pi_interaction B on A.id=B.id where 1=1 ");
        ...
        if (!StringUtils.isBlank(jsonData)) {
			...
			if (jsonObject.containsKey("entityRelated")) {
				...
				if (entityRelatedObject.containsKey("nickName") && StringUtils.isNotBlank(entityRelatedObject.getString("nickName")))
					selectBuffer.append(" and A.nick_name like '%").append(entityRelatedObject.getString("nickName")).append("%'");
	...
	public void exportExcel() {
		...
		String fileName = "pi_interaction列表数据" + DateUtils.getCurrentDateStr() + ".xls";
		String jsonData = request.getParameter(SymbolicConstant.CONTROLLER_PARAM_JSON);
