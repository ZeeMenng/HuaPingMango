...
import com.jusfoun.bll.extend.split.gp.GpEarlyWarningSplBll;
import com.jusfoun.bll.extend.unity.gp.GpEarlyWarningUntBll;
import com.jusfoun.ent.extend.gp.GpEarlyWarning;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.parameter.gp.GpEarlyWarningParameter;
 ...
 * @description 预警阀值表 对外接口，扩展自BaseSwgApp，自动生成。
 */

@Api(value = "GpEarlyWarning",tags="预警阀值表")
@RequestMapping(value = "/generate/swagger/gp/gpEarlyWarning")
public class GpEarlyWarningGenSwgApp extends BaseSwgApp {

	@Autowired
	@Qualifier("gpEarlyWarningUntBll")
	protected GpEarlyWarningUntBll gpEarlyWarningUntBll;

	@Autowired
	@Qualifier("gpEarlyWarningSplBll")
	protected GpEarlyWarningSplBll gpEarlyWarningSplBll;
	...
	@ApiImplicitParams({ @ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串，对象列表", required = true, dataType = "GpEarlyWarningAddList") })
	@RequestMapping(value = "/addList", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel addList(@RequestBody GpEarlyWarningParameter.AddList jsonData) {
		ResultModel result = gpEarlyWarningUntBll.add(jsonData.getEntityList());
	...
	@ApiImplicitParams({ @ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串，主键列表和要修改为的信息承载实体", required = true, dataType = "GpEarlyWarningUpdateList") })
	@RequestMapping(value = "/updateList", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel updateList(@RequestBody GpEarlyWarningParameter.UpdateList jsonData) {
		ResultModel result = gpEarlyWarningUntBll.updateList(jsonData);
    ...
    public ResultModel updateListWithDff(@RequestBody GpEarlyWarningParameter.AddList jsonData) {
		ResultModel result = gpEarlyWarningUntBll.updateListWithDff(jsonData.getEntityList());
    ...
    public ResultModel updateListWithDffOrAdd(@RequestBody GpEarlyWarningParameter.AddList jsonData) {
		ResultModel result = gpEarlyWarningUntBll.updateListWithDffOrAdd(jsonData.getEntityList());
	...
	public ResultModel getListByJsonData() {
        ...
        if (!StringUtils.isBlank(jsonData)) {
			...
			if (jsonObject.containsKey("entityRelated")) {
				...
				if (entityRelatedObject.containsKey("beginDate") && StringUtils.isNotBlank(entityRelatedObject.getString("beginDate")))
					selectBuffer.append(" and A.begin_date like '%").append(entityRelatedObject.getString("beginDate")).append("%'");
				if (entityRelatedObject.containsKey("endDate") && StringUtils.isNotBlank(entityRelatedObject.getString("endDate")))
					selectBuffer.append(" and A.end_date like '%").append(entityRelatedObject.getString("endDate")).append("%'");
			}
	...
	public void exportExcel() {
		...
		String fileName = "预警阀值表列表数据" + DateUtils.getCurrentDateStr() + ".xls";
		String jsonData = request.getParameter(SymbolicConstant.CONTROLLER_PARAM_JSON);
