...
import com.jusfoun.bll.extend.split.gp.GpMenuSplBll;
import com.jusfoun.bll.extend.unity.gp.GpMenuUntBll;
import com.jusfoun.ent.extend.gp.GpMenu;
import com.jusfoun.ent.custom.ResultModel;
import com.jusfoun.ent.parameter.gp.GpMenuParameter;
 ...
 * @description 链接菜单。 对外接口，扩展自BaseSwgApp，自动生成。
 */

@Api(value = "GpMenu",tags="链接菜单。")
@RequestMapping(value = "/generate/swagger/gp/gpMenu")
public class GpMenuGenSwgApp extends BaseSwgApp {
	...
	@Autowired
	@Qualifier("gpMenuSplBll")
	protected GpMenuSplBll gpMenuSplBll;

	@ApiOperation(value = "新增记录", notes = "新增单条记录")
	@ApiImplicitParams({ @ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串", required = true, dataType = "GpMenu") })
	@RequestMapping(value = "/add", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel add(@RequestBody GpMenu jsonData) {
		ResultModel result = gpMenuUntBll.add(jsonData);

		return result;
	}

	@ApiOperation(value = "批量新增", notes = "新增多条记录")
	@ApiImplicitParams({ @ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串，对象列表", required = true, dataType = "GpMenuAddList") })
	@RequestMapping(value = "/addList", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel addList(@RequestBody GpMenuParameter.AddList jsonData) {
		ResultModel result = gpMenuUntBll.add(jsonData.getEntityList());
	...
	public ResultModel delete(@RequestParam String id) {
		ResultModel result = gpMenuUntBll.delete(id);
	...
	@ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串，主键列表", required = true, dataType = "GpMenuDeleteByIdList")
	@RequestMapping(value = "/deleteList", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel deleteList(@RequestBody GpMenuParameter.DeleteByIdList jsonData) {
		ResultModel result = gpMenuUntBll.deleteByIdList(jsonData.getIdList());
    ...
    public ResultModel updateListWithDff(@RequestBody GpMenuParameter.AddList jsonData) {
		ResultModel result = gpMenuUntBll.updateListWithDff(jsonData.getEntityList());
    ...
    public ResultModel updateListWithDffOrAdd(@RequestBody GpMenuParameter.AddList jsonData) {
		ResultModel result = gpMenuUntBll.updateListWithDffOrAdd(jsonData.getEntityList());
	...
	@ApiImplicitParams({ @ApiImplicitParam(paramType = "body", name = "jsonData", value = "json字符串，查询参数", required = true, dataType = "GpMenuGetList") })
	@RequestMapping(value = "/getList", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel getList(@RequestBody GpMenuParameter.GetList jsonData) {
		// 处理业务与返回数据
		ResultModel result = gpMenuUntBll.getList(jsonData);
	...
	public ResultModel getListByJsonData() {
		...
		selectBuffer.append("select A.id id,A.domain_id domainId,A.module_id moduleId,A.name name,A.page_id pageId,A.page_url pageUrl,A.priority priority,A.remark remark,A.add_time addTime,A.update_time updateTime,A.icon_class iconClass  from gp_menu A inner join gp_menu B on A.id=B.id where 1=1 ");
