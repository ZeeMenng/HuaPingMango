...
import com.jusfoun.app.generate.swagger.gp.GpStationGenSwgApp;
...
public class GpStationSwgApp extends GpStationGenSwgApp {
	...
	public ResultModel getListByJsonData() {
		...
		Map<String, Object> map = new HashMap<String, Object>();
		StringBuffer selectBuffer = new StringBuffer();
		selectBuffer.append("select A.id id,A.serial_no serialNo,A.organization_id organizationId,A.name name,A.responsibility responsibility," + "A.remark remark,A.add_time addTime,A.update_time updateTime,C.name organizationName " + "from gp_station A inner join gp_station B on A.id=B.id LEFT JOIN gp_organization C on A.organization_id = C.id where 1=1 ");
