...
import com.jusfoun.app.generate.swagger.mf.MfForpriceGenSwgApp;
 ...
 * @description 价格预测 对外接口，扩展自MfForpriceGenSwgApp ，可手动更改。
 */
@RestController
@RequestMapping(value = "/extend/swagger/mf/mfForprice")
public class  MfForpriceSwgApp extends  MfForpriceGenSwgApp {
	...
	public ResultModel forecast() {
		...
		selectBuffer.append("		mf_forprice A                                           ");
	...
	@ApiOperation(value = "查询", notes = "市场价格-价格预测-未来一周芒果批发价格预测")
	@RequestMapping(value = "/getPriceWeek", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResultModel getPriceWeek() {
		ResultModel resultModel = new ResultModel();
		...
		selectBuffer.append(" left join (select t.* from mf_forprice t where t.region_code = '"+regionCode+"' order by substr(t.date_time,1,10) ) t1 ");
...
//			return resultModel;
//		
//		String regionCode = "";   	//地区code 1：全国, 530723：华坪县
//		String regionName = "";		//地区name 1：全国, 530723：华坪县
//		
//		JSONObject jsonObject = JSONObject.fromObject(jsonData);
//
//		if (jsonObject.containsKey("entityRelated")) {
...
//		StringBuffer selectBuffer = new StringBuffer();
//		
//		selectBuffer.append(" select t0.date_time,t0.price value from (select t.* from mf_forprice t where t.region_name like '%"+regionName+"%' order by substr(t.date_time,1,10) desc limit 26) t0");
//		
